...
- Define remaining barrier interactions with floors, hazards, broader terrain transformations, restoration, and fully scripted arena mechanisms.
...
- Runtime projectile occlusion and line-of-sight helpers now route through combined terrain-plus-barrier occlusion candidates; target awareness facts now preserve terrain occlusion source metadata. Runtime collision now pushes live physics subjects out of authored terrain collision candidates while preserving terrain source metadata. Puppet/source target facts and movement intents now query terrain traversal features: unbridged gaps produce blocked/detour path plans, while live `bridge_terrain_gap` barrier intents reopen those routes as bridged paths.
...
- Runtime barrier tile removal now recomputes terrain placement/deployment metadata so destroyed or detached attached tiles no longer leave stale terrain intents on the live barrier. Live barrier deployment now applies a first arena-state pass for destructible terrain and portals: reinforce intents raise runtime terrain HP and tag the feature, breach intents damage or remove destroyed features from the runtime terrain snapshot, and `activate_terrain_portal` intents override the owning player's selected summon portal slot from map feature metadata. Broader terrain transformation, restoration, and fully scripted arena mechanisms remain later work.
...
- `BarrierTerrainInteractionService`: evaluates placement and produces terrain interaction intents. Pure service baseline, battle/editor placement application, destructible terrain state, and first portal activation are implemented; broader arena mechanisms remain future work.
