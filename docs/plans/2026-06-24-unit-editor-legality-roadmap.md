...
12. Next: add remaining player-facing editor preview/status probes and exact slot-kind ownership fixtures when moving that rule into `scripts/services/unit_editor_legality_service.gd`.
13. Continue keeping legacy topology keys and saved-unit schema unchanged until a separate migration. Guarded by `tools/unit_editor_schema_invariance_probe.gd`, which verifies `momentum_chain_v3`, `single_unit` / `puppet_group`, `custom_topology` keys, and absence of persisted transient legality reports.
