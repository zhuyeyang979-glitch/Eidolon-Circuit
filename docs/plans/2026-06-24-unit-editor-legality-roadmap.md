...
8. Next: materialize socket-size records from live editor topology, then add main-gate probes for puppet Source Code and barrier Ether fixtures.
9. Continue keeping legacy topology keys and saved-unit schema unchanged until a separate migration. Guarded by `tools/unit_editor_schema_invariance_probe.gd`, which verifies `momentum_chain_v3`, `single_unit` / `puppet_group`, `custom_topology` keys, and absence of persisted transient legality reports.
