...
Follow-up attack target-outcome result aggregation extraction:

```text
RED: battle_hit_resolution_service_contract_probe failed because _resolve_attack still merged target_outcome killed_units inline
GREEN: BATTLE_HIT_RESOLUTION_SERVICE_CONTRACT_PROBE ok
GREEN: Godot --check-only --script res://scripts/main.gd --quit-after 1
GREEN: SNIPER_HIT_VFX_ON_TARGET_PROBE ok effects=2
GREEN: RUNTIME_CONTACT_DAMAGE_PROBE hp_delta=332 target_v=9.600
...
Follow-up editor visual selected preview extraction:

```text
RED: editor_visual_refresh_no_deep_snapshot_probe failed on missing _refresh_editor_visual_selected_part_preview helper
RED: editor_side_preview_dirty_probe returned a false green after push_error; after latching failures it failed with "Selected part change did not update side preview"
GREEN: Godot --check-only --script res://scripts/main.gd --quit-after 1
GREEN: EDITOR_VISUAL_REFRESH_NO_DEEP_SNAPSHOT_PROBE ok
...
GREEN: POSE_DRAG_NO_FULL_REFRESH_PROBE ok visual_delta=0 catalog_delta=0
GREEN: TEAMEDIT_POSE_EDIT_FRAME_BUDGET_PROBE ok root_redraw=0 component_updates=0
GREEN: TEAMEDIT_ASSEMBLY_FRAME_BUDGET_PROBE ok p95=0.72ms max=0.72ms catalog_delta=0 hot=teamedit.visual_refresh
GREEN: TEAMEDIT_HOVER_FRAME_BUDGET_PROBE ok refreshes=2 rebuilds=1
GREEN: MODULE_BINDING_GROUP_HALO_VISUAL_PROBE ok groups=2
GREEN: BOARD_ZOOM_SOCKET_FOLLOW_PROBE ok marker=0:torso_port:0 delta=34.987px
GREEN: BARRIER_TERRAIN_EDITOR_PREVIEW_PROBE ok
...
`_refresh_editor_visual_selected_part_preview()` now owns the `update_side_panels` gate for selected-part side preview refresh. `_refresh_editor_visual_views()` delegates the non-skip preview refresh with one helper call, and `_try_skip_editor_visual_refresh()` now calls the same helper before torso/engine side-panel refreshes so selected-component-only changes update side preview without forcing a board rebuild. `editor_side_preview_dirty_probe` now returns immediately after `_fail()` calls so a pushed error cannot continue into a false-green `ok` print.

Follow-up editor visual entry gate extraction:

```text
RED: editor_visual_refresh_no_deep_snapshot_probe failed on missing _begin_editor_visual_refresh helper
GREEN: Godot --check-only --script res://scripts/main.gd --quit-after 1
GREEN: EDITOR_VISUAL_REFRESH_NO_DEEP_SNAPSHOT_PROBE ok
GREEN: EDITOR_SIDE_PREVIEW_DIRTY_PROBE ok updates=3 noop=1
GREEN: EDITOR_BOARD_MODEL_INCREMENTAL_PROBE ok shallow=1 skip=1
...
GREEN: POSE_DRAG_NO_FULL_REFRESH_PROBE ok visual_delta=0 catalog_delta=0
GREEN: TEAMEDIT_POSE_EDIT_FRAME_BUDGET_PROBE ok root_redraw=0 component_updates=0
GREEN: TEAMEDIT_ASSEMBLY_FRAME_BUDGET_PROBE ok p95=0.90ms max=0.90ms catalog_delta=0 hot=teamedit.visual_refresh
GREEN: TEAMEDIT_HOVER_FRAME_BUDGET_PROBE ok refreshes=2 rebuilds=1
GREEN: MODULE_BINDING_GROUP_HALO_VISUAL_PROBE ok groups=2
GREEN: BOARD_ZOOM_SOCKET_FOLLOW_PROBE ok marker=0:torso_port:0 delta=34.987px
GREEN: BARRIER_TERRAIN_EDITOR_PREVIEW_PROBE ok
...
Follow-up editor section chrome presentation adapter extraction:

```text
RED: main_file_extraction_contract_probe failed because main.gd should centralize editor section chrome presentation application
GREEN: Godot --check-only --script res://scripts/main.gd --quit-after 1
GREEN: MAIN_FILE_EXTRACTION_CONTRACT_PROBE ok services=9
GREEN: LIFECYCLE_SERVICES_CONTRACT_PROBE ok
GREEN: UNIT_EDITOR_TEMPLATE_DRAWER_RUNTIME_PROBE ok selected=octopus barrier=BarrierTemplatepin_wall
...
`_refresh_editor_section_chrome_presentation()` now owns the main scene-tree adapter for editor section chrome: it requests one `UILifecycleService.editor_section_chrome_presentation()` plan, applies section label and template-toggle plans through `_apply_editor_control_plan()`, and preserves `_layout_editor_template_drawer()` with the planned drawer visibility. `_apply_editor_panel_visibility()` now delegates the full section chrome block with a single call.

Follow-up editor panel/role chrome presentation adapter extraction:

```text
RED: main_file_extraction_contract_probe failed because main.gd should centralize editor panel/role chrome presentation application
GREEN: Godot --check-only --script res://scripts/main.gd --quit-after 1
GREEN: MAIN_FILE_EXTRACTION_CONTRACT_PROBE ok services=9
GREEN: LIFECYCLE_SERVICES_CONTRACT_PROBE ok
GREEN: PART_LIBRARY_UI_PROBE groups=["torso", "limb", "terminal_weapon", "barrier_panel", "software_muscle", "software"] weapon=3 equipment=5 software=7 dashboard=40
...
Follow-up editor ammo-size control presentation adapter extraction:

```text
RED: main_file_extraction_contract_probe failed because main.gd should centralize editor ammo-size control presentation application
GREEN: Godot --check-only --script res://scripts/main.gd --quit-after 1
GREEN: MAIN_FILE_EXTRACTION_CONTRACT_PROBE ok services=9
GREEN: LIFECYCLE_SERVICES_CONTRACT_PROBE ok
GREEN: AMMO_SIZE_SLIDER_PROBE base=18 ranks=XS..XL ui=ok
...
`_refresh_editor_ammo_size_control_presentation()` now owns the main scene-tree adapter for editor ammo-size controls: it preserves tick/value label inputs, requests one `UILifecycleService.editor_ammo_size_control_presentation()` plan, and applies title, slider, value, and tick label plans through `_apply_editor_control_plan()`. `_apply_editor_panel_visibility()` now delegates the full ammo-size block with a single call.
