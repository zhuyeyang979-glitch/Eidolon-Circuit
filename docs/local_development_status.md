...
- Current `main.gd` size is 52,487 lines. The largest remaining functions are `_build_editor_ui` (681 lines), `_apply_editor_panel_visibility` (399), `_resolve_attack` (380), `_compute_unit_stats` (297), and `_refresh_editor_visual_views` (252).
...
- Latest focused cleanup moved action-module payload logic-field copying from `_apply_torso_slot_payload_stats()` into `UnitStatsService.apply_torso_module_payload_logic_stats()`, then moved special payload ether/soul/code classification into `UnitStatsService.apply_torso_special_payload_logic_stats()`.
