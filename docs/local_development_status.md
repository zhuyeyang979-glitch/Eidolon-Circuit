...
- Follow-up stats extraction note: torso payload direct stat merges now route through `UnitStatsService.apply_torso_payload_direct_stats()`; `main.gd` still owns slot counts, volume ranks, internal-slot status, and note formatting until a later payload model extraction.
- Follow-up verification for the torso payload direct stat merge passed: `jq empty tools/probe_manifest.json`, `git diff --check`, Godot `--check-only`, `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `thruster_dual_motion_formula_probe`, `source_code_probe`, `barrier_panel_probe`, `projectile_profile_whitelist_probe`, and `legacy_power_symbol_absence_probe`.
