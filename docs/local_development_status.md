...
- Follow-up stats extraction note: torso payload direct stat merges now route through `UnitStatsService.apply_torso_payload_direct_stats()`, sampled payload/software/ammo facts accumulate through `UnitStatsService.record_torso_payload_summary_entry()`, module payload command/module/fracture/morph/combine/identity logic-field copying routes through `UnitStatsService.apply_torso_module_payload_logic_stats()`, and final payload/ammo summary note application routes through `UnitStatsService.apply_torso_payload_summary()`; `main.gd` still owns payload traversal, catalog lookups, volume-rank sampling, special/soul/ether helper-dependent payload application, and internal-slot status lookup until a later payload model extraction.
- Follow-up verification for the torso payload direct stat merge, summary-entry accumulation, module logic-field copying, and summary extraction passed: `jq empty tools/probe_manifest.json`, `git diff --check`, Godot `--check-only`, `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `thruster_dual_motion_formula_probe`, `source_code_probe`, `barrier_panel_probe`, `projectile_profile_whitelist_probe`, and `legacy_power_symbol_absence_probe`.
...
- Current `main.gd` size is 52,492 lines. The largest remaining functions are `_build_editor_ui` (681 lines), `_apply_editor_panel_visibility` (399), `_resolve_attack` (380), `_compute_unit_stats` (297), and `_refresh_editor_visual_views` (252).
...
- Latest focused cleanup moved action-module payload logic-field copying from `_apply_torso_slot_payload_stats()` into `UnitStatsService.apply_torso_module_payload_logic_stats()`.
