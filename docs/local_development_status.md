...
- Follow-up stats extraction note: torso payload direct stat merges now route through `UnitStatsService.apply_torso_payload_direct_stats()`, sampled payload/software/ammo facts accumulate through `UnitStatsService.record_torso_payload_summary_entry()`, special payload ether/soul/code classification routes through `UnitStatsService.apply_torso_special_payload_logic_stats()`, soul heat-capacity stat/note application routes through `UnitStatsService.apply_soul_heat_capacity_stats()`, internal payload base stat accumulation routes through `UnitStatsService.apply_internal_payload_base_stats()`, module payload command/module/fracture/morph/combine/identity logic-field copying routes through `UnitStatsService.apply_torso_module_payload_logic_stats()`, final payload/ammo summary note application routes through `UnitStatsService.apply_torso_payload_summary()`, and payload slot-volume rank selection routes through `UnitStatsService.payload_slot_volume_rank()`; `main.gd` still owns payload traversal, catalog lookups, soul bonus/ether/helper-specific internal payload implementations, and internal-slot status lookup until a later payload model extraction.
...
- Resource policy note: `docs/resource_policy.md` keeps existing curated `assets/concepts/` baselines and runtime `assets/generated/` atlases in Git, keeps tracked generated `.import` metadata with those assets, and reserves ignored scratch folders for local-only art drafts.
...
- Current resource policy item is decided: no untracked `assets/concepts` or `assets/generated` files are pending, and future local-only art drafts should use ignored `_local`, `_incoming`, or `_scratch` folders unless they meet the tracked-art criteria.
...
- Follow-up repaired `editor_canvas_probe`: the stale probe mixed board-local input with a global rendering result and also used a point 16px outside the valid topology square. It now verifies the actual local-input/local-rendering contract, returns immediately on every failure, exits explicitly on success, and is registered in the manifest `core` set.
- Follow-up repaired `power_allocation_panel_duration_estimate_probe`: its old 20x-mass comparison saturated all samples at the intentional 6-second `MotionBudget` ceiling. It now tests allocation/mass monotonicity with a non-saturated fixture, tests the ceiling separately, preserves failure exit codes, and is registered in the manifest `core` set.
- Headed/manual visual verification passed on macOS Godot `4.6.2.stable.official.71f334935` with Metal Forward+: `part_identity_language_probe` and `unit_editor_assembly_template_probe` both exited `0`, generated 1280x720 screenshots, and manual inspection confirmed readable identity cards, hover detail, partial/unknown battle previews, and assembly-template slot/warning overlay. The generated PNGs were restored to the tracked baseline afterward because the assembly-template screenshot includes dynamic frame timing.
- Resource policy follow-up: `assets/concepts/` currently has 133 tracked files and is about 79M; `assets/generated/` has 20 tracked files and is about 18M. Existing curated baselines stay in Git, runtime generated atlases and their tracked `.import` metadata stay in Git, local-only art drafts go to ignored scratch folders, and Git LFS is deferred until a concrete size trigger is reached.
...
- Follow-up extraction added `UILifecycleService.editor_action_presentation()` for board-primary, unit-page, canvas/clipboard, orientation, and unit action layout/text/tooltip/color planning. `scripts/main.gd` now builds a small context dictionary and applies the returned plan as the scene-tree adapter instead of owning the long action presentation branch. The file is now 56,313 lines.
- Follow-up RED/GREEN and adjacent UI verification passed: initial `lifecycle_services_contract_probe` failed on the missing `UILifecycleService.editor_action_presentation()` method, then passed after extraction; `main_file_extraction_contract_probe` now rejects the old action presentation branch in `main.gd`. Additional passing checks: Godot `--check-only --script res://scripts/main.gd --quit-after 1`, `unit_editor_clipboard_probe`, `unit_editor_auto_connection_ui_probe`, `barrier_catalog_screen_place_probe`, `editor_board_zoom_probe`, `unit_editor_pagination_layout_probe`, `unit_editor_fullscreen_layout_probe`, `scythe_install_orientation_ui_probe`, `ammo_size_ui_probe`, `unit_editor_no_team_role_controls_probe`, `teamedit_ui_simplified_controls_probe`, `screen_layout_token_coverage_probe`, `jq empty tools/probe_manifest.json`, and `git diff --check`.
- Next safe editor UI chunk: continue splitting `_build_editor_ui` creation specs or move another focused control-update section from `_apply_editor_panel_visibility`, while keeping scene-tree ownership explicit.
...
2026-06-28 Barrier terrain destruction invalidation:

- `_remove_barrier_tile_by_index()` now recomputes live barrier terrain placement/deployment metadata after a tile is removed, so attached or bridged terrain intents cannot reference destroyed or detached barrier tiles.
- `tools/barrier_terrain_destruction_invalidation_probe.gd` is registered in `tools/probe_manifest.json`; its RED/GREEN cycle first failed on a removed `attached-tile` still referencing `wall-alpha`, then passed after removal began refreshing terrain metadata.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `barrier_terrain_destruction_invalidation_probe`, `barrier_terrain_runtime_integration_probe`, `authored_arena_terrain_runtime_probe`, `terrain_collision_runtime_probe`, `barrier_terrain_editor_preview_probe`, `barrier_terrain_map_independent_save_probe`, `battle_terrain_service_contract_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=1.654`, `max_ms=2.700`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only`.

2026-06-28 Barrier terrain entrypoint replay:

- `_apply_barrier_tile_stats()` now preserves authored barrier tile `tile_id` and `terrain_policy` when converting editor/saved `barrier_tiles` into runtime `barrier_map_tiles`, so battle deployment resolves the same terrain attach/bridge policies previewed in the editor.
- `tools/barrier_terrain_entrypoint_replay_probe.gd` is registered in `tools/probe_manifest.json`; its RED/GREEN cycle first failed with editor `shared-tile -> attach wall-alpha` while runtime produced `tile_025 -> free`, then passed after the runtime conversion retained the authored policy and stable tile ID.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `barrier_terrain_entrypoint_replay_probe`, `barrier_terrain_editor_preview_probe`, `barrier_terrain_runtime_integration_probe`, `barrier_terrain_destruction_invalidation_probe`, `barrier_terrain_map_independent_save_probe`, `authored_arena_terrain_runtime_probe`, `terrain_collision_runtime_probe`, `battle_terrain_service_contract_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=1.631`, `max_ms=2.523`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only`.

2026-06-28 Terrain-tagged field responses:

- `BattleFieldRuntimeService` now accepts terrain feature context for gravity, coolant, heat, and speed-lane field intents. Tagged terrain can provide a surface-vector gravity direction, amplify or damp gravity/heat/cooling, and mark speed lanes as connected to authored route terrain.
- Battle runtime now queries overlapping authored terrain near the relevant barrier field tiles or lane node, passes those features into the field service, and records `terrain_field_response_<kind>` metadata for diagnostics and replay probes.
- `tools/terrain_field_response_probe.gd` is registered in `tools/probe_manifest.json`; its RED/GREEN cycle first failed on missing `terrain_response`, then passed with pure service coverage and a runtime gravity field following a `gravity-route-floor` surface vector.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_field_response_probe`, `battle_field_runtime_service_contract_probe`, `barrier_terrain_runtime_integration_probe`, `barrier_terrain_editor_preview_probe`, `barrier_terrain_entrypoint_replay_probe`, `barrier_terrain_destruction_invalidation_probe`, `terrain_collision_runtime_probe`, `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe`, `battle_terrain_service_contract_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=1.637`, `max_ms=2.539`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only`.

2026-06-28 Terrain-backed path planning:

- `BattleTerrainService.traversal_query()` now evaluates source-target path corridors against authored traversal terrain, including Mobius ring shifts, unbridged `gap` blockers, hazard detours, route direction hints, and `bridge_terrain_gap` deployment intents that reopen blocked gaps as bridged paths.
- Puppet/source target awareness facts now include `terrain_path_*` metadata, and `BattleAwarenessService` penalizes blocked terrain paths while lightly favoring bridge-opened routes.
- `BattleActorCommandService.puppet_move_intent()` now consumes `terrain_path_plan`: blocked/hazard plans bend movement toward a detour vector, while bridged/route plans keep motion aligned with the route. Runtime puppets remember the active terrain path plan for diagnostics and replay probes.
- `tools/terrain_path_planning_probe.gd` is registered in `tools/probe_manifest.json`; its RED/GREEN cycle first failed on missing `traversal_query`, then passed with pure service gap/bridge coverage plus runtime puppet detour and bridge-open approach coverage.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_path_planning_probe`, `battle_terrain_service_contract_probe`, `battle_actor_command_service_contract_probe`, `battle_awareness_service_contract_probe`, `barrier_terrain_runtime_integration_probe`, `barrier_terrain_entrypoint_replay_probe`, `terrain_collision_runtime_probe`, `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe`, `terrain_field_response_probe`, `authored_arena_terrain_runtime_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=2.925`, `max_ms=12.072`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-28 Destructible terrain state pass:

- `BattleTerrainService.placement_query()` and `BarrierTerrainInteractionService` now understand map-independent `reinforce_kinds` and `breach_kinds` terrain policies, producing `reinforce_terrain_feature` and `breach_terrain_feature` deployment intents with explicit strength/damage values.
- Live barrier deployment now applies a one-time arena-state pass to the runtime terrain snapshot. Reinforce intents increase destructible terrain HP, raise max runtime terrain HP, tag the feature as `reinforced`, and record `barrier_terrain_arena_state_intents`; breach intents reduce terrain HP or remove destroyed features from the active terrain snapshot so collision/occlusion/path queries stop seeing breached terrain.
- This state application is intentionally separate from preview/recompute paths: editor preview and barrier tile removal still recompute placement/deployment metadata without replaying terrain damage.
- `tools/terrain_destructible_state_probe.gd` is registered in `tools/probe_manifest.json`; its RED/GREEN cycle first failed on missing `reinforce_terrain_feature`, then passed with pure policy/deployment coverage plus runtime reinforce and breach removal coverage.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_destructible_state_probe`, `battle_terrain_service_contract_probe`, `barrier_terrain_interaction_service_contract_probe`, `barrier_terrain_runtime_integration_probe`, `barrier_terrain_editor_preview_probe`, `barrier_terrain_map_independent_save_probe`, `barrier_terrain_entrypoint_replay_probe`, `barrier_terrain_destruction_invalidation_probe`, `terrain_path_planning_probe`, `terrain_collision_runtime_probe`, `terrain_occlusion_runtime_probe`, `terrain_field_response_probe`, `terrain_awareness_metadata_probe`, `authored_arena_terrain_runtime_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=1.719`, `max_ms=2.717`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-28 Terrain portal mechanism pass:

- `BattleTerrainService.placement_query()` and `BarrierTerrainInteractionService` now understand map-independent `portal_kinds` and `mechanism_kinds` terrain policies, producing `activate_terrain_portal` and `trigger_arena_mechanism` deployment intents without mutating saved barrier blueprints.
- The default `mobius_default_arena` terrain snapshot now includes `mobius_midfield_fold_gate`, a portal/scripted-mechanism feature whose metadata defines the summon portal slot, display name, and absolute target ring/lane.
- Live barrier deployment applies portal intents once in the arena-state pass. Activated portal features write owner-scoped runtime portal overrides, and summon feedback, HUD state, and `_spawn_for_selected_portal()` all resolve through `_portal_for_player_index()` so player-visible labels and actual spawn coordinates stay aligned.
- `BattleSpatialRuntimeService.portal_spawn_intent()` now accepts absolute portal targets for terrain portals while preserving the existing camera-relative portals and barrier camera-entry behavior.
- `tools/terrain_portal_mechanism_probe.gd` is registered in `tools/probe_manifest.json`; its RED/GREEN cycle first exposed a GDScript `String(int)` constructor error in the new resolver, then passed after the override lookup used `str(portal_slot)`.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_portal_mechanism_probe`, `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `summon_portal_deploy_logic_probe`, `terrain_destructible_state_probe`, `battle_terrain_service_contract_probe`, `barrier_terrain_interaction_service_contract_probe`, `battle_spatial_runtime_service_contract_probe`, `barrier_terrain_runtime_integration_probe`, `barrier_terrain_editor_preview_probe`, `barrier_terrain_map_independent_save_probe`, `barrier_terrain_entrypoint_replay_probe`, `barrier_terrain_destruction_invalidation_probe`, `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe`, `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=1.707`, `max_ms=2.662`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Scripted terrain mechanism state pass:

- `trigger_arena_mechanism` is now a live arena-state effect, not just a recorded deployment intent. Mechanism terrain metadata can declare `mechanism_effect`, `remove_feature_ids`, `restore_features`, and `add_features`.
- Live barrier deployment applies mechanism transforms in the same one-time arena-state pass as destructible terrain and portals. Mechanisms remove feature IDs from the runtime snapshot, normalize and upsert restored/added terrain features, increment the terrain snapshot version, and record removed/restored/added feature IDs in `barrier_terrain_arena_state_intents`.
- The runtime helper path keeps the saved blueprint map-independent: tiles still declare only `mechanism_kinds`, while actual restored terrain geometry, tags, colliders, and channels live in the active arena snapshot metadata.
- `tools/terrain_mechanism_state_probe.gd` is registered in `tools/probe_manifest.json`; it covers pure mechanism deployment intent generation plus a runtime restore switch that removes a temporary gap, restores a collision/occlusion wall, and verifies the restored wall feeds collision candidates.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_mechanism_state_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `battle_terrain_service_contract_probe`, `barrier_terrain_interaction_service_contract_probe`, `barrier_terrain_runtime_integration_probe`, `barrier_terrain_editor_preview_probe`, `barrier_terrain_map_independent_save_probe`, `barrier_terrain_entrypoint_replay_probe`, `barrier_terrain_destruction_invalidation_probe`, `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe`, `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe`, `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=1.651`, `max_ms=2.706`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Terrain hazard runtime pass:

- `BattleTerrainService.hazard_candidates()` now exposes authored terrain features whose kind, surface tags, or effect channels mark them as hazards, preserving collider, tags/channels, owner, and metadata for runtime consumers.
- Battle FIELD_SYSTEMS now applies terrain hazards to overlapping live mech physics subjects. Hazard metadata can declare `heat_rate` / `hazard_heat_rate`, `damage_per_tick` / `hazard_damage_per_tick`, `damage_per_second` / `hazard_damage_per_second`, `damage_interval`, and `damage_type`.
- Terrain hazard effects add heat through the existing heat-event path, apply periodic field damage through the existing vulnerability/material adjustment path, skip anchored barriers, and record `terrain_hazard_*` diagnostics on affected units.
- `tools/terrain_hazard_runtime_probe.gd` is registered in `tools/probe_manifest.json`; its RED/GREEN cycle first exposed invalid Godot metadata keys when raw feature IDs contained `-`, then passed after hazard timer keys were normalized.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_hazard_runtime_probe` (`heat=36.00`, `damage=5`), `battle_terrain_service_contract_probe`, `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe`, `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe`, `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `terrain_mechanism_state_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=1.848`, `max_ms=4.080`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Terrain floor surface runtime pass:

- `BattleTerrainService.surface_candidates()` now exposes authored floor/surface features whose kind, tags, or effect channels mark them as runtime surfaces, preserving collider, tags/channels, owner, metadata, and normalized orientation for FIELD_SYSTEMS consumers.
- Battle FIELD_SYSTEMS now applies terrain surface effects before terrain hazards. Surface metadata can declare `speed_mult` / `surface_speed_mult`, `response_rate` / `surface_response_rate`, `push` / `surface_push`, `cooling_rate` / `surface_cooling_rate`, `heat_rate` / `surface_heat_rate`, and `overheat_clear_ratio` / `clear_ratio`.
- Terrain surface effects clamp live unit velocity toward a stable surface-adjusted target instead of repeatedly multiplying speed, add optional directional push, route cooling/heat through the existing heat helpers, skip anchored barriers, and record `terrain_surface_*` diagnostics on affected units.
- `tools/terrain_surface_runtime_probe.gd` is registered in `tools/probe_manifest.json`; it covers pure surface candidate extraction plus runtime speed, repeated-tick stability, push, cooling, and out-of-range safety behavior.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_surface_runtime_probe` (`velocity=2.40`, `heat=16.00`), `battle_terrain_service_contract_probe`, `terrain_hazard_runtime_probe` (`heat=36.00`, `damage=5`), `terrain_field_response_probe`, `terrain_path_planning_probe`, `terrain_collision_runtime_probe` (`before=0.120`, `after=0.349`), `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe` (`clear=105.30`, `blocked=3.30`), `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `terrain_mechanism_state_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=3.228`, `max_ms=13.417`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Scripted terrain feature mutation pass:

- `trigger_arena_mechanism` metadata now supports `mutate_features` / `feature_mutations` alongside remove/restore/add. A mutation can target an existing feature by ID, set normalized feature fields, add/remove surface tags and effect channels, merge metadata, and remove metadata keys.
- Mutated features are normalized through `BattleTerrainService.normalize_feature()`, written back to the active runtime terrain snapshot, and reported through `mutated_feature_ids` in `barrier_terrain_arena_state_intents`.
- `tools/terrain_mechanism_state_probe.gd` now covers a runtime arm-switch that converts a dormant floor into an armed hazard/surface feature, then verifies the mutated feature feeds both hazard and surface candidates.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_mechanism_state_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `terrain_hazard_runtime_probe` (`heat=36.00`, `damage=5`), `terrain_surface_runtime_probe` (`velocity=2.40`, `heat=16.00`), `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe` (`before=0.120`, `after=0.349`), `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe` (`clear=105.30`, `blocked=3.30`), `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `battle_terrain_service_contract_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=1.707`, `max_ms=2.805`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Scripted terrain mechanism resource pulse pass:

- `trigger_arena_mechanism` metadata now supports `resource_delta` / `resource_gain` / `resource_pulse` as a runtime-only arena mechanism effect. The effect resolves target players through `resource_target_players`, `target_players`, or `resource_owner_scope` / `owner_scope`, then clamps each affected player's runtime resource at zero after applying the delta.
- Resource pulses are recorded in `barrier_terrain_arena_state_intents` with `runtime_effect=resource_pulse`, `target_players`, and before/after resource maps. They do not mutate the terrain feature list or increment the terrain snapshot version.
- `tools/terrain_mechanism_state_probe.gd` now covers a resource switch that grants both players resource through `resource_owner_scope=all` and verifies the unchanged snapshot version plus state-intent telemetry.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_mechanism_state_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `terrain_hazard_runtime_probe` (`heat=36.00`, `damage=5`), `terrain_surface_runtime_probe` (`velocity=2.40`, `heat=16.00`), `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe` (`before=0.120`, `after=0.349`), `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe` (`clear=105.30`, `blocked=3.30`), `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `battle_terrain_service_contract_probe`, `resource_entry_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=2.505`, `max_ms=6.538`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Scripted terrain mechanism thermal pulse pass:

- `trigger_arena_mechanism` metadata now supports `heat_delta` / `thermal_delta` / `heat_pulse`, plus `cooling_delta` for negative heat, as runtime-only arena mechanism effects. Target players resolve through `thermal_target_players`, `target_players`, or `thermal_owner_scope` / `owner_scope`.
- Thermal pulses reuse the existing heat helpers so positive pulses produce heat events and negative pulses clear heat through the normal cooling path. State telemetry records `runtime_effect=thermal_pulse`, `target_players`, `heat_delta`, and affected-unit heat before/after values without changing the terrain snapshot version.
- `tools/terrain_mechanism_state_probe.gd` now covers a P1-triggered thermal switch that heats only the enemy hero and verifies the unchanged snapshot version plus affected-unit telemetry.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_mechanism_state_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `terrain_hazard_runtime_probe` (`heat=36.00`, `damage=5`), `terrain_surface_runtime_probe` (`velocity=2.40`, `heat=16.00`), `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe` (`before=0.120`, `after=0.349`), `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe` (`clear=105.30`, `blocked=3.30`), `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `battle_terrain_service_contract_probe`, `resource_entry_probe`, `heat_event_tag_matrix_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=1.708`, `max_ms=2.972`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Scripted terrain mechanism force pulse pass:

- `trigger_arena_mechanism` metadata now supports `force_delta` / `force_pulse` / `impulse_delta` / `push_delta` as a runtime-only arena mechanism effect. Target players resolve through `force_target_players`, `target_players`, or `force_owner_scope` / `owner_scope`.
- Force pulses can use `force_radius` / `effect_radius` to limit affected units, use an explicit `force_direction` / `direction` / `force_vector`, or use `force_mode` / `direction_mode` values such as `radial_out` and `radial_in`; otherwise they fall back to the mechanism feature orientation.
- State telemetry records `runtime_effect=force_pulse`, `target_players`, `force_delta`, and affected-unit velocity before/after values without changing the terrain snapshot version. Anchored barrier terrain is excluded so deployed barrier maps do not get displaced by their own mechanism effects.
- `tools/terrain_mechanism_state_probe.gd` now covers a P1-triggered force switch that pushes only the in-radius enemy hero outward, leaves the owner ally and out-of-radius enemy unchanged, and verifies unchanged snapshot version plus affected-unit telemetry.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_mechanism_state_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `terrain_hazard_runtime_probe` (`heat=36.00`, `damage=5`), `terrain_surface_runtime_probe` (`velocity=2.40`, `heat=16.00`), `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe` (`before=0.120`, `after=0.349`), `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe` (`clear=105.30`, `blocked=3.30`), `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `battle_terrain_service_contract_probe`, `resource_entry_probe`, `heat_event_tag_matrix_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=3.128`, `max_ms=13.114`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Scripted terrain mechanism damage pulse pass:

- `trigger_arena_mechanism` metadata now supports `damage_pulse` / `mechanism_damage` / `pulse_damage` / `damage` as a runtime-only arena mechanism effect. Target players resolve through `damage_target_players`, `target_players`, or `damage_owner_scope` / `owner_scope`.
- Damage pulses can use `damage_radius` / `effect_radius` to limit affected units, `damage_type` / `mechanism_damage_type` for the field hit type, and `damage_owner_id` / `owner_id` for attribution. They reuse the existing vulnerability/material adjustment and kill-flow path, while anchored barrier terrain is excluded unless `damage_affects_anchored_barriers` is true.
- State telemetry records `runtime_effect=damage_pulse`, `target_players`, raw `damage`, `damage_type`, and affected-unit HP before/after plus final damage/block/defeat fields without changing the terrain snapshot version.
- `tools/terrain_mechanism_state_probe.gd` now covers a P1-triggered damage switch that damages only the in-radius enemy hero, leaves the owner ally and out-of-radius enemy unchanged, and verifies unchanged snapshot version plus affected-unit telemetry.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_mechanism_state_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `terrain_hazard_runtime_probe` (`heat=36.00`, `damage=5`), `terrain_surface_runtime_probe` (`velocity=2.40`, `heat=16.00`), `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe` (`before=0.120`, `after=0.349`), `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe` (`clear=105.30`, `blocked=3.30`), `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `battle_terrain_service_contract_probe`, `resource_entry_probe`, `heat_event_tag_matrix_probe`, `barrier_terrain_interaction_service_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=3.255`, `max_ms=33.551`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Scripted terrain mechanism status pulse pass:

- `trigger_arena_mechanism` metadata now supports `status_effects` / `status_pulse` / `effects`, plus direct `move_speed_mult`, `damage_mult`, and `break_value_mult` fields, as runtime-only status pulse effects. Target players resolve through `status_target_players`, `target_players`, or `status_owner_scope` / `owner_scope`.
- Status pulses use `status_radius` / `effect_radius` for range filtering and `status_duration` / `status_timer` / `duration` for timer length. They reuse the existing star-soul aura semantics: move-speed buffs/debuffs route through speed-lane or slow metadata, damage buffs/debuffs route through field-damage boost or star-soul damage debuff metadata, and break-value debuffs route through vulnerability metadata.
- Shared status ticking now decrements the star-soul aura timers and `terrain_mechanism_status_timer`, so one-shot mechanism status pulses naturally expire through the existing unit status update path.
- `tools/terrain_mechanism_state_probe.gd` now covers a P1-triggered status switch that slows, damage-debuffs, and vulnerability-marks only the in-radius enemy hero, leaves the owner ally and out-of-radius enemy unchanged, verifies unchanged snapshot version plus affected-unit telemetry, and verifies timer decay through `_update_unit_status_meta()`.
- `tools/star_soul_main_bp_flow_probe.gd` now explicitly prepares legal PVP sortie loadouts before opening the BP screen, so the probe continues testing BP completion into battle instead of being blocked by the separate empty-sortie formal-entry gate.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_mechanism_state_probe`, `star_soul_main_bp_flow_probe`, `star_soul_behavior_service_probe`, `star_soul_main_behavior_probe`, `star_soul_bp_service_probe`, `star_soul_runtime_queue_service_probe`, `star_soul_entity_service_probe`, `star_soul_main_spawn_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `terrain_hazard_runtime_probe` (`heat=36.00`, `damage=5`), `terrain_surface_runtime_probe` (`velocity=2.40`, `heat=16.00`), `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe` (`before=0.120`, `after=0.349`), `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe` (`clear=105.30`, `blocked=3.30`), `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `battle_terrain_service_contract_probe`, `barrier_terrain_interaction_service_contract_probe`, `resource_entry_probe`, `heat_event_tag_matrix_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), `battle_runtime_frame_budget_probe` (`avg_ms=4.670`, `max_ms=20.626`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Scripted terrain mechanism economy-rate pulse pass:

- `trigger_arena_mechanism` metadata now supports `economy_rate_mult` / `economy_rate_pulse` / `economy_pulse`, plus `economy_mult`, `resource_rate_mult`, `income_rate_mult`, `owner_economy_rate_mult`, and the star-soul economy stat aliases, as runtime-only economy-rate pulse effects. Target players resolve through `economy_target_players`, `target_players`, or `economy_owner_scope` / `owner_scope`.
- Economy-rate pulses store per-player runtime timers and multipliers outside the terrain snapshot, reset them on initialization and battle start, and multiply `_resource_gain_for_player()` alongside existing Star Soul economy modifiers. Resource gain uses the active fraction of each simulation delta before decrementing the terrain-mechanism timer, so short pulses do not lose their first tick.
- State telemetry records `runtime_effect=economy_rate_pulse`, `target_players`, `economy_rate_mult`, `economy_rate_duration`, and per-player before/after timer and multiplier maps without changing the terrain snapshot version.
- `tools/terrain_mechanism_state_probe.gd` now covers a P1-triggered economy switch that gives only the owner a 1.4x resource-rate pulse for 2 seconds, verifies unchanged snapshot version plus state-intent telemetry, checks resource-gain multiplication, and verifies timer decay/resets.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `terrain_mechanism_state_probe`, `star_soul_main_economy_behavior_probe`, `star_soul_behavior_service_probe`, `star_soul_main_behavior_probe`, `star_soul_bp_service_probe`, `star_soul_runtime_queue_service_probe`, `star_soul_entity_service_probe`, `star_soul_main_spawn_probe`, `star_soul_main_bp_flow_probe`, `terrain_portal_mechanism_probe`, `terrain_destructible_state_probe`, `terrain_hazard_runtime_probe` (`heat=36.00`, `damage=5`), `terrain_surface_runtime_probe` (`velocity=2.40`, `heat=16.00`), `terrain_path_planning_probe`, `terrain_field_response_probe`, `terrain_collision_runtime_probe` (`before=0.120`, `after=0.349`), `terrain_occlusion_runtime_probe`, `terrain_awareness_metadata_probe` (`clear=105.30`, `blocked=3.30`), `authored_arena_terrain_runtime_probe` (`features=4`, `collision=2`, `occlusion=2`), `battle_terrain_service_contract_probe`, `barrier_terrain_interaction_service_contract_probe`, `resource_entry_probe`, `heat_event_tag_matrix_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=258`), and `battle_runtime_frame_budget_probe` (`avg_ms=1.690`, `max_ms=2.577`).

2026-06-29 PVP timeout VP resolution probe pass:

- `tools/pvp_timeout_vp_resolution_probe.gd` is registered in `tools/probe_manifest.json`; it verifies that PVP timeout resolution honors the Star Soul loop rule that the highest VP player wins even when their remaining-health score is lower.
- The same probe locks the existing tied-VP fallback: when VP is equal at timeout, `_resolve_timeout()` uses remaining health to pick the winner, ends the battle, records `post_battle_review_reason=timeout`, and opens the post-battle review panel.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `pvp_timeout_vp_resolution_probe`, `star_soul_bp_service_probe`, `star_soul_runtime_queue_service_probe`, `star_soul_entity_service_probe`, `star_soul_behavior_service_probe`, `star_soul_main_spawn_probe`, `star_soul_main_bp_flow_probe`, `star_soul_main_cart_behavior_probe`, `star_soul_main_chaser_behavior_probe`, `star_soul_main_economy_behavior_probe`, `star_soul_main_coward_behavior_probe`, `star_soul_hud_state_probe`, `battle_controller_star_soul_runtime_probe`, `post_battle_review_loop_probe`, `battle_scoreboard_feedback_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Star Soul HUD derivation guard pass:
...
2026-06-29 Barrier terrain scout preview pass:

- Scout selected-unit details now append the same read-only barrier terrain preview used by the editor board. The detail text reports arena ID, aggregate per-tile outcomes, feature IDs, terrain kinds, and deployment intent action names so matchup scouting can reveal whether a barrier tile will attach, bridge, block, or deploy free in the active arena.
- `tools/barrier_terrain_scout_preview_probe.gd` is registered in `tools/probe_manifest.json`; it drives the real scout UI with a P2 barrier tile that attaches to `mobius_mid_cover_north`, verifies the detail text includes `attach_to_terrain` and `inherit_terrain_orientation`, and checks that neither the roster blueprint nor the arena terrain snapshot is mutated by the preview.
- README and `docs/TODO.md` now describe the implemented authored-terrain layer plus editor/scout preview surfacing instead of the old planned-only terrain wording.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `barrier_terrain_scout_preview_probe`, `barrier_terrain_editor_preview_probe`, `barrier_terrain_entrypoint_replay_probe` (`placements=1`, `deployments=2`), `barrier_terrain_map_independent_save_probe` (`tiles=1`), `barrier_terrain_interaction_service_contract_probe`, `battle_terrain_service_contract_probe`, `view_extraction_contract_probe` (`views=27`), `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Battle input-frame serialization guard pass:

- `BattleInputService` now exposes deterministic input-frame canonicalization, JSON serialization/deserialization, and replay input payload materialization. Canonical frames keep sorted `pressed`/`released` action arrays, omit false/unregistered edges when an action allowlist is supplied, and can be restored into the existing runtime edge dictionaries consumed by `action_just_pressed()` / `action_just_released()`.
- Replay input payloads now carry the current `battle_control_routes()` result, so the serialized boundary preserves local PVP P1/P2 prefixes, AI seat spectator routing, and runtime-menu input gating without adding online or rollback runtime behavior.
- `tools/battle_input_frame_serialization_probe.gd` is registered in `tools/probe_manifest.json`; it verifies order-stable serialization for equivalent input dictionaries, invalid-payload empty decoding, runtime edge consumption after restore, PVP seat-to-player route preservation, AI spectator route preservation, and menu-open route gating.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `battle_input_frame_serialization_probe`, `battle_input_service_contract_probe`, `battle_fixed_120hz_simulation_probe` (`steps=120`, `render_cap=72`), `battle_mode_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Battle start payload guard pass:

- `BattleInputService` now exposes deterministic battle-start payload materialization for the future replay/remote-input boundary. The payload carries battle mode, clamped AI/seat selection, replay seed, simulation Hz, sorted action names, the current `battle_control_routes()` result, and sorted `remote_input_slots` placeholders without changing local battle start behavior.
- `tools/battle_start_payload_probe.gd` is registered in `tools/probe_manifest.json`; it verifies stable JSON serialization for equivalent action/remote-slot orders, replay-seed and simulation-Hz preservation, PVP two-player route preservation, AI spectator route preservation, runtime-menu route gating, sorted remote input slots, and invalid JSON empty decoding.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `battle_start_payload_probe`, `battle_input_frame_serialization_probe`, `battle_input_service_contract_probe`, `battle_fixed_120hz_simulation_probe` (`steps=120`, `render_cap=72`), `battle_mode_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Local battle replay consistency guard pass:

- `tools/local_battle_replay_consistency_probe.gd` is registered in `tools/probe_manifest.json`. It creates two independent formal PVP battles from the same deterministic battle-start payload, then compares the serialized payload plus normalized battle snapshots at battle start and after twelve fixed simulation steps.
- The snapshots cover battle mode and timers, simulation counters, resources, VP, portal and sortie state, and deterministic unit identity, health, position, velocity, cooling, speed, cost, and deployment-cost data. This establishes a local initial/short-run repeatability baseline while broader full-match input replay remains deferred.
- The probe's first run exposed a runtime `String(...)` constructor error inside its unit-sort callback that could still print a false `ok`; the callback now uses `str(...)`, and the clean rerun emits no script errors.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `local_battle_replay_consistency_probe`, `battle_start_payload_probe`, `battle_input_frame_serialization_probe`, `battle_input_service_contract_probe`, `battle_fixed_120hz_simulation_probe` (`steps=120`, `render_cap=72`), `battle_mode_contract_probe`, `battle_runtime_frame_budget_probe` (`avg_ms=1.841`, `max_ms=3.504`), and `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`).

2026-06-29 Complete-lifecycle battle replay guard pass:

- `BattleInputService` input-frame schema is now version 2. Canonical frames preserve sorted pressed/released edges plus sorted, allowlisted analog/held action strengths; restored held state remains available after first-substep edges are consumed.
- `main.gd` captures complete battle input frames and routes movement, cooling, turn holds, pair summons, held aim, held melee, and gun activation through frame-backed pressed/strength readers. `_tick_battle_with_input_frame()` advances an explicit replay frame, while `_begin_battle_from_start_payload()` canonicalizes the start payload, applies its AI seat and replay seed, and records that seed for diagnostics.
- `tools/battle_full_match_replay_probe.gd` is registered in `tools/probe_manifest.json`. Its shortened two-second fixture serializes 300 fixed-step frames, runs two independent PVP matches from the same seeded payload, consumes analog movement, portal edges, and held cooling, spawns a Star Soul from a valid draft queue, reaches timeout resolution, and compares winner, VP, resource, portal, Star Soul runtime, and normalized unit terminal state.
- The registered `gun_activation_training_move_fire_probe` fixture now uses one legal Humanova topology manufacturer and reports the shared legality result on import failure. This repairs drift exposed by the later construct-body manufacturer gate while preserving its real training movement/fire coverage.
- `startup_loading_stage_probe` now explicitly reissues menu navigation after enabling automatic transitions in headless mode, matching the production rule that disables automatic loading during headless initialization while still testing the startup/menu loading task set.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `battle_full_match_replay_probe`, `local_battle_replay_consistency_probe`, `battle_input_frame_serialization_probe`, `battle_start_payload_probe`, `battle_input_service_contract_probe`, `battle_input_edge_single_consume_probe`, `tactical_input_contract_probe`, `battle_fixed_120hz_simulation_probe` (`steps=120`, `render_cap=72`), `battle_mode_contract_probe`, `star_soul_runtime_queue_service_probe`, `star_soul_entity_service_probe`, `star_soul_main_spawn_probe`, `pvp_timeout_vp_resolution_probe`, `gun_activation_training_move_fire_probe` (`moved=0.365`, `ammo=30->27`), `page_loading_transition_probe` (`transitions=4`, `completed=4`), `startup_loading_stage_probe` (`transitions=1`, `completed=1`), `battle_runtime_frame_budget_probe` (`avg_ms=3.406`, `max_ms=13.112`), `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-29 Attack-heavy replay desync diagnostic pass:

- Battle-rule timing now advances from `battle_simulation_time_seconds` at the fixed 120 Hz simulation step. Direction double taps, attack command windows, anti-stall attack age, active-melee suppression, stagger gates/combos, projectile and melee momentum gates, and repeated-module heat windows no longer depend on wall-clock execution speed.
- `BattleInputService` now materializes SHA-256 replay checkpoints from recursively canonicalized authoritative state and reports the first mismatched checkpoint, including index, simulation step, and expected/actual digests.
- `main.gd` exposes a deterministic battle checkpoint snapshot covering timer/seed, resources, VP, portals, Star Soul queue state, unit health/heat/position/velocity/action/ammo state, pending combat payload counts, attack execution telemetry, and per-player attack activation state without instance IDs or object references.
- `tools/battle_attack_heavy_replay_desync_probe.gd` is registered in `tools/probe_manifest.json`. It installs a legal two-link action-module fixture, replays 780 serialized movement/attack/turn/portal frames through two six-second PVP matches, verifies six matching checkpoints and repeated attacks, then mutates one portal edge and verifies the first divergence is reported at simulation step 360.
- The replay fixture also exposed projection fallback reads that emitted errors before a unit had stored screen-position metadata; guarded metadata reads now keep headless replay output clean.
- The broader regression pass exposed a stale `battle_hit_resolution_service_contract_probe` assertion that still expected explosive preflight to erase authored splash damage after commit `c1c72fc` intentionally preserved it. The probe now locks the preservation contract and cannot overwrite a prior failure with a final zero exit code.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `battle_attack_heavy_replay_desync_probe` (`checkpoints=6`, `desync_step=360`), `battle_full_match_replay_probe`, `local_battle_replay_consistency_probe`, `battle_input_frame_serialization_probe`, `battle_start_payload_probe`, `battle_input_service_contract_probe`, `battle_fixed_120hz_simulation_probe` (`steps=120`, `render_cap=72`), `star_soul_main_spawn_probe`, `pvp_timeout_vp_resolution_probe`, seven gun/melee activation probes, `chemical_sprayer_hold_release_probe`, `projectile_momentum_probe`, `battle_hit_resolution_service_contract_probe`, `battle_projectile_lifecycle_service_contract_probe`, `hardware_fault_gun_activation_dependency_probe`, `battle_frame_orchestrator_service_contract_probe`, `battle_runtime_facade_contract_probe`, `battle_input_edge_single_consume_probe`, `tactical_input_contract_probe`, `post_battle_review_loop_probe`, `battle_scoreboard_feedback_probe`, `battle_runtime_frame_budget_probe` (`avg_ms=1.809`, `max_ms=2.920`), and `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`).

2026-06-29 Battle replay rollback and reconnect recovery pass:

- `BattleInputService.rollback_replay_plan()` now canonicalizes recorded input history, applies unique corrected frames inside a bounded rollback window, reports the first corrected step, and returns a deterministic replay-from-start plan. Invalid history, duplicate corrections, future steps, and stale corrections are rejected with explicit reasons.
- Versioned reconnect payloads now carry the canonical battle-start payload, canonical Star Soul draft queue, confirmed serialized input history, and confirmed checkpoint. A SHA-256 digest covers the complete recovery core.
- `reconnect_replay_plan()` validates reconnect schema, start/draft structure, frame count, checkpoint coverage, fixed-step checkpoint time, checkpoint digest format, and the complete history digest before exposing replay data. Structurally invalid serialized history frames are rejected instead of being silently normalized into empty input.
- `docs/plans/2026-06-29-battle-replay-recovery-design.md` records the input-history recovery architecture and keeps network transport, live rollback scheduling, and scene-object snapshots explicitly out of scope.
- `tools/battle_rollback_replay_probe.gd` is registered in `tools/probe_manifest.json`. It proves a missing remote portal edge diverges, correcting steps 75/76 converges to the 180-step baseline, and a correction outside the configured window is rejected.
- `tools/battle_reconnect_replay_probe.gd` is registered in `tools/probe_manifest.json`. It rebuilds the exact step-90 state, continues to the same step-180 state, rejects tampered history through its digest, and rejects an under-covering checkpoint.
- Verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `battle_rollback_replay_probe` (`rollback_step=75`, `frames=180`), `battle_reconnect_replay_probe` (`step=90`), `battle_input_service_contract_probe`, `battle_input_frame_serialization_probe`, `battle_start_payload_probe`, `local_battle_replay_consistency_probe`, `battle_full_match_replay_probe`, `battle_attack_heavy_replay_desync_probe` (`checkpoints=6`, `desync_step=360`), `battle_fixed_120hz_simulation_probe` (`steps=120`, `render_cap=72`), `star_soul_main_spawn_probe`, `battle_runtime_frame_budget_probe` (`avg_ms=1.846`, `max_ms=3.824`), `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), `jq empty tools/probe_manifest.json`, `git diff --check`, and Godot `--check-only --quit-after 1`.

2026-06-30 UnitStatsService torso payload processing-plan pass:

- `UnitStatsService` now owns payload slot-key normalization, generic volume-rank value normalization, and `torso_payload_processing_plan()` for the torso-slot payload route model. The plan returns summary-entry facts plus direct-stat/internal-merge/special/module route keys without touching catalog lookup or runtime nodes.
- `main.gd` now delegates the repeated `_apply_torso_slot_payload_stats()` branch routing to the service plan, while preserving existing `_payload_part_for_payload()` catalog selection, exact `_payload_slot_volume_rank()` sampling, `_merge_internal_payload_stats()` callbacks, soul bonus callbacks, and ether callbacks in the adapter layer.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.torso_payload_processing_plan()`, then passed after the service and `main.gd` delegation were added.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `thruster_dual_motion_formula_probe`, `source_code_probe`, `soul_oath_activation_probe`, `ether_heat_economy_probe`, `barrier_panel_probe`, `projectile_profile_whitelist_probe`, `legacy_power_symbol_absence_probe`, `engine_momentum_allocation_open_probe`, `power_allocation_panel_heat_live_update_probe`, `internal_slot_size_probe`, and `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`).

2026-06-30 UnitStatsService slot-volume rank formula pass:

- `UnitStatsService.part_slot_volume_rank()` now owns exact part slot-volume rank thresholds for explicit `slot_volume_tier`, shield/electronic-armor payloads, engine output/mass, cooling output/mass, booster boost momentum/mass, generic footprint, and limb footprint reduction.
- `main.gd` keeps `_part_slot_volume_rank()` as a compatibility adapter that supplies the existing exact size-tier rank and booster boost-momentum facts from local helpers, then delegates the rank formula to the service.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.part_slot_volume_rank()`, then passed after the service and adapter delegation were added. `tools/main_file_extraction_contract_probe.gd` now also requires the new delegation token.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `internal_slot_size_probe`, `thruster_dual_motion_formula_probe`, `engine_momentum_allocation_open_probe`, `power_allocation_panel_heat_live_update_probe`, `source_code_probe`, `soul_oath_activation_probe`, `ether_heat_economy_probe`, `barrier_panel_probe`, `projectile_profile_whitelist_probe`, and `legacy_power_symbol_absence_probe`.

2026-06-30 UnitStatsService internal slot planner pass:

- `UnitStatsService` now owns internal plugin slot acceptance, torso internal slot size-list materialization, and best-slot selection. The service accepts plain part/context dictionaries plus occupied-slot facts, so it can be tested without scene/editor state.
- `main.gd` keeps `_internal_slot_accepts_payload()`, `_torso_internal_slot_size_ranks()`, and `_best_internal_slot_for_payload()` as compatibility adapters. The torso adapter passes the existing plugin capacity and torso size-rank facts into the service.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.internal_slot_accepts_payload()`, then passed after the service and `main.gd` delegation were added. `tools/main_file_extraction_contract_probe.gd` now requires the three new delegation tokens.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `internal_slot_size_probe`, `ammo_install_size_payload_probe`, `shield_payload_slot_volume_probe`, `torso_slot_capacity_contract_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `engine_momentum_allocation_open_probe`, and `power_allocation_panel_heat_live_update_probe`.

2026-06-30 UnitStatsService torso payload plan executor pass:

- `UnitStatsService.apply_torso_payload_plan()` now owns the pure execution glue for a torso payload plan: summary accumulation, direct payload stat application, special payload field application, hero soul heat-capacity application, and module payload logic copying.
...
2026-06-30 UnitStatsService soul bonus stat pass:

- `UnitStatsService.apply_soul_bonus_stats()` now owns ordinary soul base bonuses, conditional soul bonus fields, duelist-oath echo/activation fields, oath-only pierce/speed bonuses, and the oath recovery/stability modifiers.
- `main.gd` keeps `_apply_soul_bonus()` as a compatibility adapter that supplies body requirement and duelist-oath failure facts from the existing blueprint/topology helpers.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.apply_soul_bonus_stats()`, then passed after the service method and `main.gd` delegation were added. The same probe verifies active/inactive ordinary souls plus active/inactive duelist-oath souls.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, Godot `--check-only --quit-after 1`, `soul_oath_activation_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `source_code_probe`, `ether_heat_economy_probe`, `internal_slot_size_probe`, `barrier_panel_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), and `battle_runtime_frame_budget_probe` (`avg_ms=4.100`, `max_ms=12.252`).

2026-06-30 UnitStatsService payload context pass:

- `UnitStatsService.part_payload_context()` now owns ammo metadata copying, torso payload context dictionary assembly, legacy mass-limit volume-rank derivation, and torso slot-volume tier rank derivation.
- `UnitStatsService.torso_payload_context()` now owns the shared direct-payload ammo context used by torso payload direct stat application.
- `main.gd` keeps `_unit_stats_part_payload_context()` and `_unit_stats_torso_payload_context()` as compatibility adapters that supply project-level ammo constants and existing torso capacity facts.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.part_payload_context()`, then passed after the service methods and `main.gd` delegations were added. The same probe verifies duplicated ammo metadata, adapter torso capacity preservation, non-torso fallback, and rank derivation.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, Godot `--check-only --quit-after 1`, `internal_slot_size_probe`, `ammo_install_size_payload_probe`, `shield_payload_slot_volume_probe`, `torso_slot_capacity_contract_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `engine_momentum_allocation_open_probe`, `power_allocation_panel_heat_live_update_probe`, `ether_heat_economy_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), and `battle_runtime_frame_budget_probe` (`avg_ms=1.886`, `max_ms=4.063`).

2026-06-30 UnitStatsService payload slot-volume rank pass:

- `UnitStatsService.payload_slot_volume_rank()` now owns payload volume-rank selection for ammo tier overrides, precomputed context overrides, and fallback slot rank lookup.
- `main.gd` keeps `_payload_slot_volume_rank()` as a compatibility adapter that supplies exact local size-tier rank and booster boost-momentum facts to the service.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.payload_slot_volume_rank()`, then passed after the service method and `main.gd` delegation were added. `tools/main_file_extraction_contract_probe.gd` now requires the new delegation token.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, Godot `--check-only --quit-after 1`, `internal_slot_size_probe`, `ammo_install_size_payload_probe`, `shield_payload_slot_volume_probe`, `torso_slot_capacity_contract_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `engine_momentum_allocation_open_probe`, `power_allocation_panel_heat_live_update_probe`, `ether_heat_economy_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), and `battle_runtime_frame_budget_probe` (`avg_ms=1.767`, `max_ms=4.582`).

2026-06-30 UnitStatsService part size-tier rule pass:

- `UnitStatsService` now owns size-tier label normalization, rank mapping, footprint thresholds, and part-level tier inference through five pure APIs. The service preserves the existing `starter`/`nano` through `leviathan` aliases and exact footprint boundaries as the single source of truth.
- The compatibility helpers in `main.gd` now delegate to the service, and `_payload_slot_volume_rank()` / `_part_slot_volume_rank()` no longer calculate or pass size-tier context. Only the still-local booster momentum fact remains in those adapters.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.normalize_size_tier_label()`, then passed after the service implementation. Both extraction probes now require the size-rule delegations and reject the removed adapter-side rank expression.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `unit_editor_topology_socket_size_gate_probe`, `catalog_card_size_badge_probe`, `part_identity_contract_probe`, `internal_slot_size_probe`, `ammo_install_size_payload_probe`, `shield_payload_slot_volume_probe`, `torso_slot_capacity_contract_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `source_code_probe`, `torso_size_mass_probe`, `part_size_visual_probe`, `unit_editor_socket_size_rejection_probe`, `terminal_weapon_size_probe`, `engine_momentum_allocation_open_probe`, `power_allocation_panel_heat_live_update_probe`, `ether_heat_economy_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), `battle_runtime_frame_budget_probe` (`avg_ms=1.762`, `max_ms=5.247`), and Godot `--check-only --quit-after 1`.

2026-06-30 UnitStatsService thruster momentum rule pass:

- `UnitStatsService` now owns the economy median-mass table, default boost duration/multiplier, drive-demand precedence, move/boost efficiency clamps, normal thrust, Boost extra thrust, and total Boost momentum. `main.gd` keeps its public compatibility helpers and constants but delegates them to the service.
- Booster slot-volume rank now derives exact total Boost momentum inside `UnitStatsService`; `_payload_slot_volume_rank()` and `_part_slot_volume_rank()` are context-free service calls, completing removal of their adapter-side size/booster fact construction.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.economy_median_mass_for_rank()`, then passed after the rule family moved. Both extraction probes require all seven main-side delegations and reject any remaining `booster_boost_momentum` adapter field.
- Follow-up migrated `thruster_fixed_drive_demand_probe` and `thruster_dual_budget_legality_probe` from scrubbed `engine_momentum_required` / `thruster_allocated_momentum` reads to canonical `drive_demand_total` assertions, and migrated `thruster_philosophy_probe` from cross-size averages and legacy recoil cancellation to explicit family-default tradeoffs plus catalog mass identity. All three now preserve failure exit status and are registered in the manifest `core` set.
- The same follow-up repaired `power_allocation_panel_boost_dash_probe`: it now uses the real engine-payload detail-open path instead of reading a deliberately closed view, returns immediately after failures, and is registered in `core`.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `boost_formula_allocation_plus_extra_probe`, `boost_formula_v3_probe`, `thruster_momentum_range_probe`, `thruster_gradient_catalog_probe`, `thruster_gradient_v3_probe`, `thruster_dual_gradient_probe`, `thruster_dual_motion_formula_probe`, `thruster_dual_allocation_range_probe`, `thruster_dual_slider_writeback_probe`, `thruster_hover_boost_terms_probe`, `thruster_momentum_motion_probe`, `thruster_same_power_chain_probe`, `thruster_turn_momentum_probe`, `engine_thruster_cooling_economy_probe`, `engine_thruster_limb_budget_probe`, `internal_slot_size_probe`, `ammo_install_size_payload_probe`, `shield_payload_slot_volume_probe`, `editor_cost_accounting_probe`, `engine_momentum_allocation_open_probe`, `power_allocation_panel_heat_live_update_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), `battle_runtime_frame_budget_probe` (`avg_ms=1.787`, `max_ms=2.759`), and Godot `--check-only --quit-after 1`.

2026-06-30 UnitStatsService payload catalog selection pass:

- `UnitStatsService.payload_catalog_selection()` now owns payload-kind slot mapping, legacy index-key selection, exact `part_name` / `component_name` restoration, missing-name index fallback, and ammo-size variant intent as a pure catalog-selection descriptor.
- `main.gd::_payload_part_for_payload()` now only supplies the role catalog, materializes the normalized selected component, and applies the existing ammo variant adapter when requested. Eight repeated per-kind name/index lookup branches were removed.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing selection API, then passed after the service implementation. The service contract covers saved special/module names, missing engine names, ammo variants, muscle payloads, and custom slots; both extraction probes require the delegation and reject the removed direct saved-name lookup.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `ammo_install_size_payload_probe`, `shield_payload_slot_volume_probe`, `internal_slot_size_probe`, `editor_cost_accounting_probe`, `unit_editor_engine_allocation_service_contract_probe`, `unit_editor_assembly_template_service_contract_probe`, `source_code_priority_main_runtime_probe`, `soul_oath_activation_probe`, `ether_heat_economy_probe`, `module_payload_rebind_keeps_module_probe`, `single_booster_per_torso_install_probe`, `payload_requires_torso_install_probe`, `torso_slot_capacity_contract_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), `battle_runtime_frame_budget_probe` (`avg_ms=1.922`, `max_ms=4.204`), and Godot `--check-only --quit-after 1`.

2026-06-30 UnitStatsService torso capacity rule pass:

- `UnitStatsService` now owns torso identity, brain-torso identity, torso slot size-rank, baseline plugin/software capacity, plugin capacity, software capacity, and context-free internal slot size-list materialization. The service preserves the existing baseline-plus-one and clamp-to-12 behavior while making the slot profile helper derive its default capacity/rank without adapter context.
- `main.gd` keeps `_component_is_torso()`, `_component_is_brain_torso()`, `_torso_size_rank_for_slots()`, `_torso_baseline_slot_capacity()`, `_torso_plugin_capacity_for_part()`, `_torso_software_capacity_for_part()`, and `_torso_internal_slot_size_ranks()` as compatibility wrappers, all delegated to `UnitStatsService`.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.component_is_torso()`, then passed after the rule family moved. Both extraction probes now require the new delegations and reject the old inline torso capacity formula strings in `main.gd`.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `jq empty tools/probe_manifest.json`, `git diff --check`, Godot `--check-only --quit-after 1`, `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `torso_slot_capacity_contract_probe`, `torso_slot_capacity_plus_one_probe`, `internal_slot_size_probe`, `ammo_install_size_payload_probe`, `shield_payload_slot_volume_probe`, `editor_cost_accounting_probe`, `unit_build_rule_training_gate_probe`, `unit_editor_engine_allocation_service_contract_probe`, `source_code_priority_main_runtime_probe`, `soul_oath_activation_probe`, `ether_heat_economy_probe`, `module_payload_rebind_keeps_module_probe`, `single_booster_per_torso_install_probe`, `payload_requires_torso_install_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), and `battle_runtime_frame_budget_probe` (`avg_ms=1.787`, `max_ms=3.604`).

2026-06-30 UnitStatsService internal payload callback pass:

- `UnitStatsService` now owns concrete internal payload stat merge callbacks for engine payloads, cooling payloads, and booster/thruster payloads: engine output/budget/tag/role/profile/weighted family fields, cooling tags/manual relief/overheat/rate/dissipation fields, and thruster drive/boost/heat/duration/profile/family fields.
- `main.gd` keeps `_merge_engine_stats()`, `_merge_cooling_profile_stats()`, `_merge_thruster_drive_stats()`, `_cooling_tags_for_part()`, and `_movement_profile_priority()` as compatibility wrappers that supply local defaulting/allocation facts, then delegate the pure dictionary mutation to `UnitStatsService`.
- `tools/unit_stats_service_contract_probe.gd` first failed on the missing `UnitStatsService.cooling_tags_for_part()`, then passed after the callback family moved. Both extraction probes now require the new main-side delegations and reject the old inline engine/cooling/thruster merge statements in `main.gd`.
- `tools/engine_philosophy_probe.gd` and `tools/engine_runtime_stat_merge_probe.gd` had stale calls to the old three-argument `_apply_engine_momentum_budget()` shape; they now use `_apply_engine_momentum_budget_with_runtime_modifiers()` and run cleanly.
- Focused verification passed on macOS Godot `4.6.2.stable.official.71f334935`: `jq empty tools/probe_manifest.json`, `git diff --check`, Godot `--check-only --quit-after 1`, `unit_stats_service_contract_probe`, `main_file_extraction_contract_probe`, `engine_momentum_unification_probe`, `engine_philosophy_probe`, `engine_runtime_stat_merge_probe`, `engine_output_current_x3_probe`, `engine_output_triple_probe`, `thermal_chain_v3_probe`, `cooler_pool_double_probe`, `boost_heat_independent_from_drive_allocation_probe`, `boost_peak_no_idle_heat_probe`, `boost_formula_allocation_plus_extra_probe`, `boost_formula_v3_probe`, `thruster_dual_motion_formula_probe`, `thruster_dual_allocation_range_probe`, `thruster_dual_slider_writeback_probe`, `internal_slot_size_probe`, `ammo_install_size_payload_probe`, `shield_payload_slot_volume_probe`, `editor_cost_accounting_probe`, `engine_momentum_allocation_open_probe`, `power_allocation_panel_heat_live_update_probe`, `unit_build_rule_training_gate_probe`, `probe_manifest_no_legacy_fixture_probe` (`current=259`, `sections=8`), and `battle_runtime_frame_budget_probe` (`avg_ms=1.838`, `max_ms=4.681`).
