...
- A detached `ce7e436` baseline reproduced the stale assertions from `thruster_fixed_drive_demand_probe` and `thruster_philosophy_probe`; `thruster_dual_budget_legality_probe` reports the same obsolete no-limb engine-demand assumption. These probes are outside the current registered gate set, and `thruster_momentum_*` is already listed under `obsolete_review`, so none are used as completion evidence.
