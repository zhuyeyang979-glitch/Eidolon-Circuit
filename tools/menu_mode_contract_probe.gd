...
func _init() -> void:
	...
	if String(enter_intent.get("mode_key", "")) != "menu" or bool(enter_intent.get("bound", true)):
		...
		return
	var enter_trimmed: Dictionary = mode.enter_intent("  startup  ", {"boot": true})
	if String(enter_trimmed.get("reason", "")) != "startup":
		_fail("MenuMode enter_intent should normalize reason: %s" % str(enter_trimmed))
		return
	...
	if not bool(show_now.get("should_apply", false)) or bool(show_now.get("loading_queued", true)) or not bool(show_now.get("preloaded", false)):
		...
		return
	var show_trimmed: Dictionary = mode.show_intent(true, false, "  ")
	if String(show_trimmed.get("reason", "")) != "menu":
		_fail("MenuMode show_intent should normalize blank reason: %s" % str(show_trimmed))
		return
	...
	if String(exit_trimmed.get("reason", "")) != "navigate":
		_fail("MenuMode exit_intent should normalize reason: %s" % str(exit_trimmed))
		return
