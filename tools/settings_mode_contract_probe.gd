...
func _init() -> void:
	...
	if String(show_now.get("category_key", "")) != "input" or not bool(show_now.get("should_apply", false)) or String(show_now.get("return_target", "")) != "battle":
		...
		return
	var show_trimmed: Dictionary = mode.show_intent("  input  ", true, "  battle  ", false, "  ")
	if String(show_trimmed.get("category_key", "")) != "input" or String(show_trimmed.get("return_target", "")) != "battle" or String(show_trimmed.get("reason", "")) != "settings":
		_fail("SettingsMode show_intent should normalize category, return target, and blank reason: %s" % str(show_trimmed))
		return
