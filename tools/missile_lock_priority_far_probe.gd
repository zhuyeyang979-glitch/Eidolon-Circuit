...
func _init() -> void:
	...
	var event := {"direction": Vector2.RIGHT, "range": 3.4, "missile_lock_range": 3.4, "missile_lock_cone_degrees": 60.0, "missile_lock_priority": "far_first"}
	if main._acquire_missile_lock_target(attacker, event) != far:
		_fail("far_first should select the farthest legal target.")
	print("MISSILE_LOCK_PRIORITY_FAR_PROBE ok")
	quit()
