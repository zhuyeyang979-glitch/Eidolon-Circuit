...
func _init() -> void:
	...
	for raw_entry in Array(after.get("entries", [])):
		...
		if not ["booster_drive", "booster_boost_brake", "limb"].has(String(entry.get("kind", ""))):
			continue
		var actual := float(entry.get("momentum", -9999.0))
		var min_m := maxf(0.0, float(entry.get("min_momentum", 0.0)))
		var max_m := maxf(min_m, float(entry.get("max_momentum", min_m)))
		if actual < min_m - 0.02 or actual > max_m + 0.02:
			_fail("%s outside range %.2f..%.2f: %.2f" % [String(entry.get("id", "")), min_m, max_m, actual])
		checked += 1
	if checked < 4:
		_fail("Not enough entries checked: %d" % checked)
	print("POWER_ALLOCATION_EQUALIZE_CLAMP_PROBE ok checked=%d" % checked)
	quit()
