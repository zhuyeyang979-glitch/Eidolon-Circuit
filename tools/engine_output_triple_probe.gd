...
func _init() -> void:
	...
	if absf(main._engine_momentum_output_for_part(explicit) - 90.0) > 0.001:
		_fail("Explicit engine output is not scaled by the current 9x effective multiplier.")
	var budget := {"engine_momentum_budget": 7.0}
	if absf(main._engine_momentum_output_for_part(budget) - 63.0) > 0.001:
		_fail("Legacy engine budget diagnostic output is not scaled by the current 9x effective multiplier.")
