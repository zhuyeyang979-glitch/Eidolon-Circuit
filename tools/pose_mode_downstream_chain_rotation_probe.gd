...
func _first_terminal(main: Node) -> int:
	for i in range(main._catalog_for("hero", "muscle").size()):
...
func _init() -> void:
	...
	var nodes: Array = []
	var edges: Array = []
	var torso := main._append_component_root_node(nodes, "CORE", Vector2(0.40, 0.5), _first_torso(main))
	var limb_a := main._append_directed_component_node("hero", unit_bp, nodes, edges, torso, "A", "limb_muscle", 0, Vector2.RIGHT, [0])
	var limb_b := main._append_directed_component_node("hero", unit_bp, nodes, edges, limb_a, "B", "limb_muscle", 1, Vector2.RIGHT, [0])
	var terminal := main._append_directed_component_node("hero", unit_bp, nodes, edges, limb_b, "TIP", "muscle", _first_terminal(main), Vector2.RIGHT, [0])
	...
	var before_limb_a: Vector2 = main._topology_node_position(nodes[limb_a])
	var before_limb_b: Vector2 = main._topology_node_position(nodes[limb_b])
	var before_terminal: Vector2 = main._topology_node_position(nodes[terminal])
	var click_pos := _visible_point_far_from_center(main, unit_bp, limb_b)
	main._handle_editor_board_input(_mouse_button(click_pos, true))
	if not bool(main.editor_pose_dragging) or int(main.editor_pose_root_node) != limb_b:
		_fail("Middle limb did not start pose drag: %s" % String(main.last_pose_drag_reject_reason))
	var drag_pos := _drag_target_around_pivot(main, unit_bp, limb_b, click_pos, 0.72)
	...
	if main._topology_node_position(nodes[torso]).distance_to(before_torso) > 0.00001:
		_fail("Pose subtree drag moved torso.")
	if main._topology_node_position(nodes[limb_a]).distance_to(before_limb_a) > 0.00001:
		_fail("Pose subtree drag moved upstream limb.")
	if main._topology_node_position(nodes[limb_b]).distance_to(before_limb_b) <= 0.001:
		_fail("Pose subtree drag did not move root limb.")
	if main._topology_node_position(nodes[terminal]).distance_to(before_terminal) <= 0.001:
		_fail("Pose subtree drag did not move downstream terminal.")
	main._handle_editor_board_input(_mouse_button(drag_pos, false))
	print("POSE_MODE_DOWNSTREAM_CHAIN_ROTATION_PROBE ok")
	quit()
