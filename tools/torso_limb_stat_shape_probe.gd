...
func _init() -> void:
	...
	var forearm_profile: Dictionary = main._embedded_joint_profile_for_part(forearm, "limb_muscle")
	var thigh_profile: Dictionary = main._embedded_joint_profile_for_part(thigh, "limb_muscle")
	var steel_profile: Dictionary = main._embedded_joint_profile_for_part(steel, "limb_muscle")
	var colossus_profile: Dictionary = main._embedded_joint_profile_for_part(colossus, "limb_muscle")
	...
	_expect_gt("thigh drive over forearm", float(thigh_profile.get("output_momentum", 0.0)), float(forearm_profile.get("output_momentum", 0.0)))
	_expect_gt("steel drive over thigh", float(steel_profile.get("output_momentum", 0.0)), float(thigh_profile.get("output_momentum", 0.0)))
	_expect_gt("colossus drive over steel", float(colossus_profile.get("output_momentum", 0.0)), float(steel_profile.get("output_momentum", 0.0)))
