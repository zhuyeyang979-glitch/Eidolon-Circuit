...
func _init() -> void:
	...
	fighter._ready()
	fighter.setup_unit({"owner_id": 1, "role": "hero", "unit_name": "HammerPose", "stats": stats})
	...
	action["timer"] = float(action.get("duration", 0.82)) * 0.55
	fighter.runtime_module_actions[0] = action
	var moving_axis := _axis(_target_segment(fighter, target_node))
	if absf(rad_to_deg(base_axis.angle_to(moving_axis))) < 14.0:
		_fail("Hammer Windup-Slam should visibly rotate the hammer terminal.")
	if float(event.get("runtime_contact_speed", 0.0)) <= 0.01:
		_fail("Hammer Windup-Slam should publish runtime contact speed.")
	print("HAMMER_WINDUP_SLAM_RUNTIME_POSE_PROBE ok")
	quit()
