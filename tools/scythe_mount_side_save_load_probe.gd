...
func _build_unit(main: Node, unit_name: String) -> Dictionary:
	...
	var limb: int = main._append_directed_component_node("hero", unit_bp, nodes, edges, torso, "HANDLE", "limb_muscle", _first_limb(main), Vector2.RIGHT)
	var scythe: int = main._append_directed_component_node("hero", unit_bp, nodes, edges, limb, "LEFT SCYTHE", "muscle", _scythe_index(main), Vector2.RIGHT)
	var node: Dictionary = Dictionary(nodes[scythe]).duplicate(true)
	node["visual_mount_side"] = "left"
