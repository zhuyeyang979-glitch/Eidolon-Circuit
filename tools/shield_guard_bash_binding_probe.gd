...
func _init() -> void:
	...
	if shield_index < 0 or hammer_index < 0:
		_fail("Shield or hammer catalog target missing.")
	var legal := ProbeLib.build_unit(main, shield_index, module_index, "shield_terminal", "SHIELD")
	...
	if legal_bindings.size() != 1 or not bool(Dictionary(legal_bindings[0]).get("runtime_valid", false)):
		_fail("Shield terminal should bind Shield Guard-Bash.")
	var invalid := ProbeLib.build_unit(main, hammer_index, module_index, "shield_terminal", "HAMMER")
	...
	if invalid_bindings.is_empty() or bool(Dictionary(invalid_bindings[0]).get("runtime_valid", true)):
		_fail("Hammer terminal must not bind Shield Guard-Bash.")
	print("SHIELD_GUARD_BASH_BINDING_PROBE ok")
	quit()
