...
func _init() -> void:
	...
	if not bool(show_now.get("should_apply", false)) or String(show_now.get("focus_path", "")) != "user://saved_units/a.json" or String(show_now.get("return_context", "")) != "editor":
		...
		return
	var show_trimmed: Dictionary = mode.show_intent("  user://saved_units/a.json  ", "  editor  ", false, true, false)
	if String(show_trimmed.get("focus_path", "")) != "user://saved_units/a.json" or String(show_trimmed.get("return_context", "")) != "editor":
		_fail("SavedUnitsMode show_intent should trim focus path and return context: %s" % str(show_trimmed))
		return
	var trimmed_payload: Dictionary = Dictionary(show_trimmed.get("payload", {}))
	if String(trimmed_payload.get("focus_path", "")) != "user://saved_units/a.json":
		_fail("SavedUnitsMode show_intent payload should use trimmed focus path: %s" % str(show_trimmed))
		return
