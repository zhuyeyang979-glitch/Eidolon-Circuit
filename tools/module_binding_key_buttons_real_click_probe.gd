...
func _init() -> void:
	...
	var button: Button = main.editor_action_buttons["bind_key_4"]
	if not button.visible or button.disabled:
		_fail("Real bind key button 4 is not clickable after target selection.")
	button.pressed.emit()
	var bindings: Array = Array(unit_bp.get("module_bindings", []))
	if bindings.size() != 1:
		_fail("Real bind key button did not finalize the module binding.")
	var binding: Dictionary = Dictionary(bindings[0])
	if int(binding.get("attack_key", 0)) != 4:
		_fail("Real bind key button wrote the wrong attack key.")
	if not main.editor_pending_module_binding.is_empty():
		_fail("Pending module binding should clear after real key button click.")
	print("MODULE_BINDING_KEY_BUTTONS_REAL_CLICK_PROBE ok")
	quit()
