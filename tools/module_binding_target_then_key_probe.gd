...
func _init() -> void:
	...
	if bool(main.editor_action_buttons["bind_key_1"].visible):
		_fail("Attack-key buttons should stay hidden before a target is selected.")
	...
	if not bool(main.editor_pending_module_binding.get("target_selected", false)):
		_fail("Target selection did not enter target-selected binding state.")
	if not bool(main.editor_action_buttons["bind_key_1"].visible):
		_fail("Attack-key buttons should appear after a valid target is selected.")
	main._set_pending_module_attack_key(6)
	var bindings: Array = unit_bp.get("module_bindings", [])
	if bindings.size() != 1 or int(Dictionary(bindings[0]).get("attack_key", 0)) != 6:
		_fail("Target-then-key flow did not save the selected attack key.")
	if not main.editor_pending_module_binding.is_empty():
		_fail("Pending binding state should clear after final key selection.")
	if failed:
		quit(1)
		return
	print("MODULE_BINDING_TARGET_THEN_KEY_PROBE ok")
	quit()
