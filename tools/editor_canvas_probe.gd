...
func _init() -> void:
	...
	var reference_topology_point := Vector2(0.68, 0.42)
	var board_point: Vector2 = main._topology_position_to_board_local(reference_topology_point)
	var topo_point: Vector2 = main._board_position_to_topology(board_point)
	var roundtrip: Vector2 = main._topology_position_to_board_local(topo_point)
	if roundtrip.distance_to(board_point) > 2.0:
		_fail("Canvas local roundtrip drifted %.2f px, input and display mapping are not unified." % roundtrip.distance_to(board_point))
		return
