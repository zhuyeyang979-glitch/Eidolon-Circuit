...
func _init() -> void:
	...
	for token in [
		...
		"func snapshot",
		"func reset",
		"last_cleanup_preserved",
		"last_cleanup_clear_runtime",
		"last_preserve_for_return",
	]:
	...
	if String(preserve_snapshot.get("phase", "")) != "preserved" or int(preserve_snapshot.get("preserve_count", 0)) != 1:
		...
		return
	var preserve_cleanup_state = BattleStateScript.new()
	preserve_cleanup_state.begin_runtime("ai", "battle:ai", {"mode": "ai"})
	preserve_cleanup_state.record_cleanup_intent({"preserve": true, "clear_runtime": false, "reason": "preserve_for_return", "categories": []})
	var preserve_cleanup_snapshot: Dictionary = preserve_cleanup_state.snapshot()
	if String(preserve_cleanup_snapshot.get("phase", "")) != "preserved" or not bool(preserve_cleanup_snapshot.get("last_cleanup_preserved", false)) or not bool(preserve_cleanup_snapshot.get("last_preserve_for_return", false)):
		_fail("BattleState preserve cleanup visibility mismatch: %s" % str(preserve_cleanup_snapshot))
		return
	if bool(preserve_cleanup_snapshot.get("last_cleanup_clear_runtime", true)) or String(preserve_cleanup_snapshot.get("last_cleanup_reason", "")) != "preserve_for_return":
		_fail("BattleState preserve cleanup flags mismatch: %s" % str(preserve_cleanup_snapshot))
		return
	var cleanup_snapshot: Dictionary = state.record_cleanup_intent({"preserve": false, "clear_runtime": true, "reason": "exit_battle", "categories": ["units"]})
	...
	if String(state_snapshot.get("phase", "")) != "cleanup" or int(state_snapshot.get("cleanup_count", 0)) != 1:
		...
		return
	if bool(state_snapshot.get("last_cleanup_preserved", true)) or not bool(state_snapshot.get("last_cleanup_clear_runtime", false)) or bool(state_snapshot.get("last_preserve_for_return", true)):
		_fail("BattleState clear cleanup visibility mismatch: %s" % str(state_snapshot))
		return
	if String(state_snapshot.get("last_cleanup_reason", "")) != "exit_battle" or Array(state_snapshot.get("last_cleanup_categories", [])).size() != 1:
		_fail("BattleState clear cleanup reason/category mismatch: %s" % str(state_snapshot))
		return
	state.reset()
	if String(state.phase) != "idle" or state.reset_count != 0 or not state.last_runtime_snapshot.is_empty() or bool(state.last_cleanup_preserved):
