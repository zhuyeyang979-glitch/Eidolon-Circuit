...
func _build_bound_unit(main: Node, side: String) -> Dictionary:
	...
	var limb: int = main._append_directed_component_node("hero", unit_bp, nodes, edges, torso, "HANDLE", "limb_muscle", 0, Vector2.RIGHT)
	var scythe: int = main._append_directed_component_node("hero", unit_bp, nodes, edges, limb, "%s SIDE SCYTHE" % side.to_upper(), "muscle", _scythe_index(main), Vector2.RIGHT)
	var node: Dictionary = Dictionary(nodes[scythe]).duplicate(true)
	node["visual_mount_side"] = side
	...
	if candidate.is_empty() or not bool(candidate.get("valid", false)):
		_fail("Scythe module candidate should be valid for %s mount side: %s" % [side, String(candidate.get("reason", ""))])
		return {}
	...
	if not main._complete_pending_module_binding_with_selection(unit_bp, Array(candidate.get("selection", [scythe]))):
		_fail("Could not select scythe binding target for %s mount side." % side)
		return {}
	main.editor_pending_module_binding["attack_key"] = 1
	if not main._finalize_pending_module_binding_after_key(unit_bp):
		_fail("Could not finalize scythe binding for %s mount side." % side)
		return {}
	return unit_bp
...
func _assert_runtime_action(main: Node, unit_bp: Dictionary, side: String) -> void:
	...
	if event.is_empty():
		_fail("Scythe module did not start for %s mount side." % side)
		return
	if bool(event.get("projectile", false)):
		_fail("Scythe module produced projectile for %s mount side." % side)
		return
	...
	if String(segment.get("visual_mount_side", "")) != side:
		_fail("Runtime scythe segment lost %s mount side." % side)
		return
	if String(segment.get("orientation_category", "")) != "orthogonal_side_mount":
		_fail("Runtime scythe segment lost orthogonal side-mount category.")
		return
	fighter.queue_free()
...
func _init() -> void:
	...
	print("SCYTHE_MODULE_BINDING_MOUNT_SIDE_PROBE ok")
	quit()
