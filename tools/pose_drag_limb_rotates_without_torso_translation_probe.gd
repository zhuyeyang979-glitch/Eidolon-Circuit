...
func _init() -> void:
	...
	var limb_pos: Vector2 = main._topology_node_position(nodes[limb])
	...
	if not main._start_editor_pose_drag(unit_bp, limb, start_local, [limb, terminal]):
		_fail("Could not start pose drag for limb chain.")
	...
	if main._topology_node_position(moved_nodes[torso]).distance_to(torso_pos) > 0.00001:
		_fail("Pose limb drag translated the torso.")
	if main._topology_node_position(moved_nodes[limb]).distance_to(limb_pos) <= 0.00001:
		_fail("Pose limb drag did not rotate/move the root limb.")
	if main._topology_node_position(moved_nodes[terminal]).distance_to(terminal_pos) <= 0.00001:
		_fail("Pose limb drag did not carry the terminal downstream.")
	var gap := main._topology_max_socket_gap("hero", unit_bp, moved_nodes, edges)
	if gap > 0.00001:
		_fail("Pose limb drag created socket gap %.6f." % gap)
	if Dictionary(unit_bp.get("entry_pose", {})).is_empty():
		_fail("Pose drag did not store entry_pose.")
	print("POSE_DRAG_LIMB_ROTATES_WITHOUT_TORSO_TRANSLATION_PROBE ok")
	quit()
