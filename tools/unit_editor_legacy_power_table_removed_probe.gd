...
func _assert_detail_closed(main, label: String) -> void:
	if main.engine_momentum_allocation_view != null and main.engine_momentum_allocation_view.visible:
		_fail("%s unexpectedly opened the allocation detail panel." % label)
...
func _init() -> void:
	...
	_assert_detail_closed(main, "non-explicit context refresh")
	if main.editor_power_dock_view == null or not main.editor_power_dock_view.visible:
		_fail("New power allocation dock is not visible after context refresh.")
	if Array(main.editor_power_dock_view.entries).size() < 2:
		_fail("New power allocation dock did not receive thruster/limb entries.")
	if main.editor_engine_allocation_torso_node_index < 0:
		_fail("Power allocation context was not preserved.")
	print("UNIT_EDITOR_LEGACY_POWER_TABLE_REMOVED_PROBE ok non_explicit_detail_closed=true dock_entries=%d" % Array(main.editor_power_dock_view.entries).size())
	quit()
