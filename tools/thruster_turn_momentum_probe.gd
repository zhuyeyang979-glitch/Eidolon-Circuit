...
func _stats_for_mass(mass: float) -> Dictionary:
	return {
		"mass": mass,
		"speed_mult": 1.0,
		"thruster_drive_demand": 96.0,
		"thruster_allocated_momentum": 9999.0,
		"thruster_boost_extra_demand": 192.0,
		"thruster_boost_peak_demand": 288.0,
		"engine_momentum_output": 288.0,
		"move_efficiency": 1.0,
		"turn_efficiency": 1.0,
...
func _init() -> void:
	...
	var expected_turn_accel := clampf(
		float(light.get("thruster_effective_drive_demand", 0.0)) / maxf(1.0, float(light.get("mass", 1.0)))
	...
	main._apply_engine_momentum_budget(no_thruster, "hero")
