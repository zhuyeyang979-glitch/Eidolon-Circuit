...
func _init() -> void:
	...
	var expected_turn_accel := clampf(
		float(light.get("thruster_momentum", 0.0)) / maxf(1.0, float(light.get("mass", 1.0)))
	...
	var no_thruster := {"mass": 16.0, "speed": 3.0, "thruster_momentum": 0.0, "boost_momentum": 0.0}
