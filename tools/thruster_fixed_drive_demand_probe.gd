...
func _init() -> void:
	...
	if absf(float(stats.get("thruster_drive_demand", -1.0)) - expected) > 0.01:
		...
		return
	if stats.has("thruster_allocated_momentum") or stats.has("engine_momentum_required"):
		_fail("Computed stats should expose canonical drive fields instead of legacy momentum fields.")
		return
	if absf(float(stats.get("drive_demand_total", -1.0)) - (expected + expected_boost_brake)) > 0.01:
		_fail("Canonical drive demand should include fixed drive plus boost/brake demand when no limbs are bound.")
		return
	print("THRUSTER_FIXED_DRIVE_DEMAND_PROBE ok demand=%.1f total=%.1f" % [expected, float(stats.get("drive_demand_total", 0.0))])
	quit(0)
