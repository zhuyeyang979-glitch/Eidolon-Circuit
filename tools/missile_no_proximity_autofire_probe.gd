...
func _init() -> void:
	...
	root.add_child(main)
	main._ready()
	var seeker = _make_unit("MISSILE_SEEKER", 1, true)
	var target = _make_unit("MISSILE_TARGET", 2, false)
	seeker.deploy(5.0, 0.0)
	target.deploy(5.55, 0.0)
	main.all_units = [seeker, target]
	main.active_units = {1: {"hero": seeker, "barrier": null, "puppet": []}, 2: {"hero": target, "barrier": null, "puppet": []}}
	...
	print("MISSILE_NO_PROXIMITY_AUTOFIRE_PROBE ok")
	quit()
