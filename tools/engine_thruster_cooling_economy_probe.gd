	...
	for i in range(catalog.size()):
		...
		if slot_key == "engine":
			if float(part.get("power", 0.0)) + 0.01 < main._economy_engine_power_target(rank):
				_fail("Engine power below economy target: %s" % String(part.get("name", "")))
			if float(part.get("idle_heat", 0.0)) <= 0.0:
				_fail("Engine lacks explicit idle heat: %s" % String(part.get("name", "")))
		elif slot_key == "booster":
			var expected_move_momentum := median * 2.0 * MainScene.THRUSTER_MOMENTUM_MULT
			var expected_boost_momentum := median * 4.0 * MainScene.THRUSTER_MOMENTUM_MULT
			for field in ["thruster_family", "thruster_momentum", "thruster_duration", "boost_momentum", "boost_duration", "idle_heat", "brake_efficiency", "recoil_cancel", "flame_color", "summary"]:
			...
			if main._booster_normal_momentum_for_part(part) <= 0.0:
				_fail("Booster lacks explicit normal momentum: %s" % String(part.get("name", "")))
			if main._booster_boost_momentum_for_part(part) <= 0.0:
				_fail("Booster lacks explicit boost momentum: %s" % String(part.get("name", "")))
			...
			if main._booster_normal_momentum_for_part(part) + 0.01 >= expected_move_momentum and main._booster_boost_momentum_for_part(part) + 0.01 >= expected_boost_momentum:
				booster_target_ranks[rank] = true
			var stats := {"role": "hero", "mass": median, "thruster_momentum": main._booster_normal_momentum_for_part(part), "boost_momentum": main._booster_boost_momentum_for_part(part), "boost_duration": 0.3, "speed_mult": 1.0, "engine_motion_scale": 1.0}
...
func _init() -> void:
	...
	for rank in range(1, 6):
		var engine := {"name": "E", "slot_volume_tier": main._volume_rank_label(float(rank)), "power": 0, "mass": 1}
		...
		if float(engine.get("power", 0.0)) < main._economy_engine_power_target(rank):
			_fail("Synthetic same-size engine target failed rank %d." % rank)
		if float(cooling.get("cooling", 0.0)) < main._engine_idle_heat_for_part(engine, float(engine.get("power", 0.0))) + main._booster_idle_heat_for_part(booster):
