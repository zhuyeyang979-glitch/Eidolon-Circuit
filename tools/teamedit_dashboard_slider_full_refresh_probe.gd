...
func _init() -> void:
	...
	if limb_id == "":
		_fail("Missing limb allocation entry.")
	main.editor_update_ui_count = 0
	main.editor_allocation_full_refresh_count = 0
	main._set_engine_momentum_allocation_ratio(limb_id, 0.31)
	...
	var expected := main._engine_allocation_clamped_momentum_from_entries(Array(data.get("entries", [])), limb_id, pool * 0.31, pool)
	var found := false
	for value in by_node.values():
		if absf(float(value) - expected) <= 0.01:
			found = true
	if not found:
		_fail("Slider did not write clamped allocation %.3f to limb binding." % expected)
	if int(main.editor_allocation_full_refresh_count) <= 0:
		_fail("Slider finish did not trigger dashboard allocation full refresh.")
