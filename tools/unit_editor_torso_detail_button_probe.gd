...
func _init() -> void:
	...
	if not main.editor_torso_detail_button.visible:
		_fail("Dedicated torso detail button is not visible in Unit Edit.")
	...
	if main.editor_open_torso_node_index != torso_node:
		_fail("Torso detail opened wrong node: %d expected %d." % [main.editor_open_torso_node_index, torso_node])
	main._toggle_dashboard_torso_detail()
	...
	print("UNIT_EDITOR_TORSO_DETAIL_BUTTON_PROBE ok torso=%d" % torso_node)
	quit()
