...
func _init() -> void:
	...
	if mode.app_mode_key_for_battle_mode("training") != "training" or mode.app_mode_key_for_battle_mode("ai") != "battle":
		...
		return
	if mode.app_mode_key_for_battle_mode("  training  ") != "training":
		_fail("BattleMode app mode mapping should normalize battle mode keys.")
		return
	...
	if String(show_training.get("mode_key", "")) != "training":
		...
		return
	var show_training_trimmed: Dictionary = mode.show_intent("  training  ", true, false, "battle:training")
	if String(show_training_trimmed.get("mode_key", "")) != "training" or String(show_training_trimmed.get("battle_mode", "")) != "training":
		_fail("BattleMode training show should normalize battle mode key: %s" % str(show_training_trimmed))
		return
	if String(Dictionary(show_training_trimmed.get("payload", {})).get("mode", "")) != "training":
		_fail("BattleMode training show payload should use normalized mode: %s" % str(show_training_trimmed))
		return
	...
	if not mode.commit_cleanup(cleanup_preserve):
		...
		return
	var preserve_cleanup_state: Dictionary = mode.state_snapshot()
	if String(preserve_cleanup_state.get("phase", "")) != "preserved" or not bool(preserve_cleanup_state.get("last_cleanup_preserved", false)) or not bool(preserve_cleanup_state.get("last_preserve_for_return", false)):
		_fail("BattleMode preserve cleanup should be visible in state snapshot: %s" % str(preserve_cleanup_state))
		return
	...
	if not mode.commit_cleanup(cleanup_clear):
		...
		return
	var clear_cleanup_state: Dictionary = mode.state_snapshot()
	if String(clear_cleanup_state.get("phase", "")) != "cleanup" or bool(clear_cleanup_state.get("last_cleanup_preserved", true)) or not bool(clear_cleanup_state.get("last_cleanup_clear_runtime", false)):
		_fail("BattleMode clear cleanup should be visible in state snapshot: %s" % str(clear_cleanup_state))
		return
	...
	if not mode.commit_enter(enter_intent) or mode.enter_count != 1:
		...
		return
	var training_enter_intent: Dictionary = mode.enter_intent("battle:training", {"mode": "  training  "})
	if String(training_enter_intent.get("mode_key", "")) != "training":
		_fail("BattleMode enter_intent should normalize payload mode: %s" % str(training_enter_intent))
		return
