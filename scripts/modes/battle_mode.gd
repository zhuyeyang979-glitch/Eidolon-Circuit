...
func app_mode_key_for_battle_mode(battle_mode_key: String) -> String:
	return TRAINING_MODE_KEY if battle_mode_key == TRAINING_MODE_KEY else MODE_KEY
...
func show_intent(battle_mode_key: String, preloaded: bool, loading_queued: bool, reason: String = "", preserve_runtime: bool = false) -> Dictionary:
	...
	if resolved_reason == "":
		resolved_reason = "return_battle" if preserve_runtime else "battle:%s" % battle_mode_key
	return {
		"mode_key": app_mode_key_for_battle_mode(battle_mode_key),
		"battle_mode": battle_mode_key,
		"reason": resolved_reason,
		"payload": {"mode": battle_mode_key, "preserve_runtime": preserve_runtime} if preserve_runtime else {"mode": battle_mode_key},
		"preloaded": preloaded,
...
func enter_intent(reason: String = "", payload: Dictionary = {}) -> Dictionary:
	return {
		"mode_key": app_mode_key_for_battle_mode(String(payload.get("mode", ""))),
		"reason": reason,
