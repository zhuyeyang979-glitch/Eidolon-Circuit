...
const BODY_STEPS := [
	{
		...
		"zh_title": "核心",
		"en_title": "CORE",
		"zh_instruction": "先放核心，确定硬件端口、体量和承载空间。",
		"en_instruction": "Start with a core to set hardware ports, size, and capacity.",
	},
	{
		"key": "joint_muscle",
		...
		"zh_title": "连接件",
		"en_title": "CONNECTOR",
		"zh_instruction": "再接连接件，让核心拥有可驱动的硬件结构。",
		"en_instruction": "Add connectors so the core has driven hardware structure.",
	},
	{
		"key": "weapon",
		"slot_key": "muscle",
	...
	},
	{
		...
		"zh_title": "引擎",
		"en_title": "ENGINE",
		"zh_instruction": "安装引擎，给推进、连接件链和行动模块提供动力预算。",
		"en_instruction": "Install an engine to power movement, connector chains, and actions.",
	},
	{
		"key": "cooling",
		"core_concept": "heat",
...
const BARRIER_STEPS := [
	{
		...
		"zh_title": "功能模块",
		"en_title": "FUNCTION",
		"zh_instruction": "先放功能模块，确定固定空间和屏障覆盖。",
		"en_instruction": "Start with function modules to define space and coverage.",
	},
	{
		"key": "identity",
		"slot_key": "special",
...
const TUTORIAL_ZH_BY_KEY := {
	"torso": {
		"function": "核心是英雄的承载硬件，决定硬件接口数量、体量、软硬件槽和软件槽。",
		"limit": "接口数量限制可连接硬件；软硬件/软件槽限制引擎、散热器、推进器、弹药箱和行动模块容量。",
		"next": "点“前往”筛选核心，把一个运动核心或神经核心拖到画布，作为第一次组装的根节点。",
	},
	"joint_muscle": {
		"function": "连接件把核心延伸成可驱动的硬件结构，是武器和动作的力臂。",
		"limit": "连接件要和接口贴合；长度、材料和承载会影响后续武器、动力分配和连接合法性。",
		"next": "选择一段基础连接件，拖到核心附近，之后用自动连接或手动接口把它接上。",
	},
	"weapon": {
		"function": "武器决定主要伤害方式：近战靠接触姿态，枪械靠弹药、射程和行动模块触发。",
		"limit": "多数末端武器只有一个接口；侧挂刃需要选择左右方向，枪械还要匹配弹药和动作模块。",
		"next": "选择一种新手易读的末端武器，放在构件链末端，先保证结构清楚再追求复杂组合。",
	},
	"connection": {
	...
	"engine": {
		"function": "引擎提供动力预算，驱动移动、推进器、绑定硬件链和部分行动模块。",
	...
	"module": {
		...
		"limit": "模块需要合法目标、键位和软件槽；绑定硬件链会消耗动力，并可能改变热节奏。",
		"next": "安装一个简单行动模块，选择目标与攻击键，最后保存单位或进入训练测试。",
	},
	"barrier_panel": {
		"function": "功能模块决定固定空间、屏障覆盖和以太构件的基础形状。",
		"limit": "结界格数量和材料槽有限；部分功能模块只能放在屏幕格或自由画布的合法位置。",
		"next": "先放一个清晰的功能模块，再补身份核心和主动模块。",
	},
}
...
const TUTORIAL_EN_BY_KEY := {
	...
	"joint_muscle": {
		...
		"next": "Drag one beginner connector near the core, then link it manually or with Auto Connect.",
	},
	"weapon": {
	...
	"engine": {
		"function": "Engines provide drive budget for movement, boosters, bound connector chains, and some action modules.",
	...
	"module": {
		...
		"limit": "Modules need legal targets, keys, and software slots; bound connector chains consume drive and can change heat tempo.",
		"next": "Install one simple action, choose its target and key, then save or enter Training.",
	},
	"barrier_panel": {
		"function": "Function modules define fixed space, coverage, and the base shape for Ether parts.",
		"limit": "Barrier cells and material slots are limited; some function modules only fit screen cells or valid board positions.",
		"next": "Place one clear function module, then add identity and active modules.",
	},
}
