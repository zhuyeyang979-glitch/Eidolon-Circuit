...
func draw_overlay(canvas: CanvasItem, canvas_size: Vector2, model: Dictionary, zh: bool) -> void:
	if model.is_empty():
		return
	var panel_width := clampf(canvas_size.x * 0.46, 318.0, 392.0)
	if canvas_size.x < 360.0:
		panel_width = maxf(260.0, canvas_size.x - 24.0)
	var panel_height := 174.0
	var panel_y := clampf(84.0, 16.0, maxf(16.0, canvas_size.y - panel_height - 48.0))
	var panel_pos := Vector2(clampf((canvas_size.x - panel_width) * 0.5, 12.0, maxf(12.0, canvas_size.x - panel_width - 12.0)), panel_y)
	var panel := Rect2(panel_pos, Vector2(panel_width, panel_height))
	var status := String(model.get("status", "ok"))
	var status_color := _status_color(status)
	canvas.draw_rect(panel, Color(0.015, 0.035, 0.052, 0.82), true)
	...
	canvas.draw_string(font, panel.position + Vector2(12.0, 19.0), String(model.get("title", "")), HORIZONTAL_ALIGNMENT_LEFT, panel.size.x * 0.42, 13, Color(0.88, 0.97, 1.0, 0.96))
	var status_label := "可用" if zh else "READY"
	if status == "warn":
		status_label = "待处理" if zh else "CHECK"
	elif status == "missing":
		status_label = "缺核心" if zh else "NO CORE"
	canvas.draw_string(font, panel.position + Vector2(panel.size.x - 76.0, 19.0), status_label, HORIZONTAL_ALIGNMENT_RIGHT, 64.0, 12, status_color)
	var schema_rect := Rect2(panel.position + Vector2(12.0, 36.0), Vector2(86.0, 76.0))
	...
	var grid_x := schema_rect.position.x + schema_rect.size.x + 10.0
	var grid_width := panel.position.x + panel.size.x - grid_x - 12.0
	var cell_gap := 5.0
	var cell_w := maxf(56.0, (grid_width - cell_gap * 2.0) / 3.0)
	var cell_h := 27.0
	for i in range(mini(slots.size(), 6)):
		...
		var rect := Rect2(Vector2(grid_x + col * (cell_w + cell_gap), panel.position.y + 36.0 + row * (cell_h + 5.0)), Vector2(cell_w, cell_h))
...
func _draw_warnings(canvas: CanvasItem, panel: Rect2, model: Dictionary, zh: bool) -> void:
	...
	canvas.draw_string(font, panel.position + Vector2(12.0, warning_y + 10.0), warning_title, HORIZONTAL_ALIGNMENT_LEFT, 64.0, 10, Color(0.78, 0.9, 1.0, 0.68))
	if warnings.is_empty():
		canvas.draw_string(font, panel.position + Vector2(70.0, warning_y + 10.0), "全部已安排" if zh else "All placed", HORIZONTAL_ALIGNMENT_LEFT, panel.size.x - 84.0, 10, Color(0.32, 1.0, 0.72, 0.82))
		return
	var warning_limit := mini(warnings.size(), 4)
	for i in range(warning_limit):
		...
		var y := warning_y + 9.0 + i * 14.0
		canvas.draw_circle(panel.position + Vector2(76.0, y - 3.5), 2.6, dot_color)
		var line := "%s: %s" % [
			_trim(String(warning.get("label", "")), 7),
			_trim(String(warning.get("detail", "")), 14),
		]
		canvas.draw_string(font, panel.position + Vector2(84.0, y), line, HORIZONTAL_ALIGNMENT_LEFT, panel.size.x - 100.0, 10, Color(0.92, 0.96, 1.0, 0.84))
...
func _draw_slot(canvas: CanvasItem, rect: Rect2, slot: Dictionary) -> void:
	...
	var font := ThemeDB.get_fallback_font()
	var label := _trim(String(slot.get("label", "")), 5)
	var value := _trim(String(slot.get("value", "")), 5)
	canvas.draw_string(font, rect.position + Vector2(8.0, 11.0), label, HORIZONTAL_ALIGNMENT_LEFT, rect.size.x - 30.0, 9, Color(0.78, 0.9, 1.0, 0.84))
	canvas.draw_string(font, rect.position + Vector2(rect.size.x - 24.0, 19.0), value, HORIZONTAL_ALIGNMENT_RIGHT, 20.0, 12, color)
